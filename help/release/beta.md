...
これにより、マーチャンダイジングの柔軟性と業務効率が向上し、より適切な製品体験を大規模に提供できるようになります。 詳しくは、[ マーチャンダイジングルール ](https://experienceleague.adobe.com/en/docs/commerce/optimizer/merchandising/rules/add)を参照してください。
...
この機能強化により、複雑なコマース環境をまたいでレコメンデーションを管理することが簡素化され、より適切な商品発見体験を提供できるようになります。 詳しくは、[推奨事項](https://experienceleague.adobe.com/en/docs/commerce/optimizer/manage-results/recommendation-performance)を参照してください
...
### Category Merchandising （パブリックBeta）

[!BADGE SaaSのみ]{type=Positive url="https://experienceleague.adobe.com/en/docs/commerce/user-guides/product-solutions" tooltip="Adobe Commerce as a Cloud ServiceおよびAdobe Commerce Optimizer プロジェクト（Adobeが管理するSaaS インフラストラクチャ）にのみ適用されます。"}

Adobe Commerce Optimizerのカテゴリーマーチャンダイジングは、カテゴリーページで商品を注文する方法を販売者が制御できるようにします。 マーチャンダイジングルールワークフローの&#x200B;**カテゴリルール**&#x200B;として、検索ルール [およびデフォルトの製品リストルール ](https://experienceleague.adobe.com/en/docs/commerce/optimizer/merchandising/rules/overview)と共に動作を設定します。 各ルールは、**インテリジェントなランキング** （行動シグナルとAIが該当する場合）、オプションの&#x200B;**価格ベースの並べ替え**、ピン、ブースト、埋め込みなどの&#x200B;**手動** アクションを組み合わせることができます。これにより、外部ツールを使用せずに、発見をキュレートし、プロモーションを実行し、カテゴリーページを戦略に合わせることができます。

**主なメリット**

- 検索とデフォルトのリストに使用されるのと同じルールベースのワークフローを使用して、1つ以上のカテゴリをターゲットにします。
- インテリジェントなランキング戦略（例：最も閲覧された商品、最も購入された商品、カートに追加された商品、パーソナライズされたレコメンデーション、トレンド商品など）と、オプションの価格ソートを使用して、カテゴリーリストを注文できます。
- 正確な配置が必要な場合、インテリジェントなランキングの上に手動ピン、ブースト、埋め込みアクションを重ねます。
- 設定で複数のカタログビューを使用する場合、カタログビューごとにルールをスコープ化します。

詳しくは、[ カテゴリーマーチャンダイジング ](https://experienceleague.adobe.com/en/docs/commerce/optimizer/merchandising/rules/add)を参照してください。
...
[Cloud Automation Patching Service](../tools/caps-tool/intro.md)は、Cloud Infrastructure[環境の](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/overview)Adobe Commerceに個別のセキュリティ パッチを適用するプロセスを自動化します。

...
<table style="table-layout:auto">
    <tbody>
        ...
        <tr>
           ...
           <td>このリリースには、次のユーザー受注管理の機能拡張が含まれています。
             <ul>
              ...
              <li><!--LYNX-642--><code>order.availableActions</code> フィールドを更新して、マーチャントが管理者で並べ替え機能を <a href="https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/shopper-tools/reorders-allow#configure-to-allow-customer-reorders"> 無効 </a> にした場合に <code>REORDER</code> アクションが返されないようにします。</li>
        ...
        <td>
            <ul>
              <li><!--LYNX-730-->カタログ価格ルールのストア設定を追加し、使用可能なすべてのカタログルールを取得する <a href="https://developer.adobe.com/commerce/webapi/graphql/schema/products/queries/all-rules/"><code>allCatalogRules</code></a> クエリを追加し、製品に現在適用されているカタログルールを表示する <code>Product.rules</code> フィールドを公開しました。</li>

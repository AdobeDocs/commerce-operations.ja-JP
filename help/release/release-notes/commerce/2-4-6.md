...
Adobe Commerce 2.4.6-beta2 は、次のコンポーネントバージョンに対してテストされています。
...
Adobe Commerce 2.4.6-beta2 では、PHP 8.2 のサポートが導入されています。PHP 8.1 は完全にサポートされています。  <!-- AC-6598 -->

PHP 7.4 のサポートは、このリリースから削除されました。 PHP 7.4 ではAdobe Commerceを実行できません。

Adobe Commerce 2.4.6-beta2 では、以下をサポートするようになりました。
...
Adobe Commerce 2.4.6 のコアコードで発生していた数百の問題を修正しています。 Adobe Commerce 2.4.6-beta2 に含まれる修正済みの問題のサブセットを以下に示します。
...
* GraphQLクエリの応答に、バンドル製品の子製品に関する情報が `product` セクション `options` ブロック
...
* 顧客 `order` クエリで、指定したすべての Web サイト、ストア、ストアの表示からの注文が期待どおりに返されるようになりました。 この修正には、新しいリクエスト引数の作成が含まれます。

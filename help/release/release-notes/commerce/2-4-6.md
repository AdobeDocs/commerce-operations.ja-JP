...
Adobe Commerce 2.4.6-beta は、次のコンポーネントバージョンに対してテストされています。
...
Adobe Commerce 2.4.6 では、PHP 8.2 のサポートが導入されています。PHP 8.1 は完全にサポートされています。 <!-- AC-6598 -->

PHP 7.4 のサポートは、このリリースから削除されました。 PHP 7.4 ではAdobe Commerceを実行できません。

Adobe Commerce 2.4.6-beta5 では、次の機能がサポートされるようになりました。
...
**カテゴリ権限が有効なカテゴリに対するクエリを実行する際の応答時間を改善しました**. カテゴリ権限が有効になっていて、多数の顧客グループまたは共有カタログを含むカテゴリ内の製品に対するクエリを実行する際の応答時間が改善されました。 GraphQLのリゾルバーの操作も最適化されました。 <!-- B2B-2179 2404 2329-->

**カテゴリツリーレンダリングのパフォーマンスの最適化**. この `categoryList` クエリは、多数のカテゴリを持つ顧客をサポートするように最適化されています。 このクエリを使用して 1500 を超えるカテゴリを検索し、最大 5 レベルのネストされたサブカテゴリを含めると、パフォーマンスが向上します。 不要なメソッド呼び出しを削除し、カテゴリツリーのキャッシュを改善し、カテゴリデータを再帰的に読み込むことで、リファクタリングコードによるカテゴリの子の読み込みを改善しました。 <!-- B2B-2484 2469 2472-->
...
**製品の権限が最適化されました** を製品インラインフラグメントと共に使用します。 <!-- B2B-2459-->
...
Adobe Commerce 2.4.6 のコアコードで発生していた数百の問題を修正しています。 Adobe Commerce 2.4.6-beta に含まれる修正済みの問題のサブセットを以下に示します。
...
* カテゴリに割り当てられていないが、バンドルまたはグループ化された製品の製品オプションである製品は、 **カテゴリの参照を許可** 設定は、顧客がクエリを送信する際に有効になります。
...
* 構成可能な製品数量の要件を含む買い物かごルールが、期待どおりに動作するようになりました。 `attribute_scope` 対象 `quote_item_qty` 条件が次の値に変更されました： `parent`. [GitHub-35981](https://github.com/magento/magento2/issues/35981)
...
* 内部サーバーエラー `/V1/products/{sku}/links` API エンドポイントが解決されました。 [GitHub-35923](https://github.com/magento/magento2/issues/35923)

<!-- AC-6421-->

* 内部サーバーエラー `/V1/creditmemo/refund` API エンドポイントが解決されました。 [GitHub-35925](https://github.com/magento/magento2/issues/35925)

<!-- AC-7497-->

* `get.php` これ以降、 `resource_config.json` が破損しています： `Trying to access array offset on value of type null`. [GitHub-36640](https://github.com/magento/magento2/issues/36640)

<!-- AC-6440-->

* 固定バージョンの PHP コードスニファーの依存関係をMagento2 プロジェクトのメインから削除しました。 `composer.json` ファイル。 [GitHub-35994](https://github.com/magento/magento2/issues/35994)

<!-- AC-7300-->

* の不要な操作を削除しました。 `version` 内からのキー `sampledata:deploy`. 以前は、 `bin/magento sampledata:deploy` は、 `version` キーが見つかりませんでした `composer.json`. コマンドが正常に実行されると、Adobe Commerceが成功を確認し、次に実行する手順を示すようになりました。 [GitHub-36554](https://github.com/magento/magento2/issues/36554)
...
* Adobe Commerceは、無効なフォーム入力でエラーメッセージを重複しなくなりました。 [GitHub-36201](https://github.com/magento/magento2/issues/36201)

<!-- AC-7508-->

* 管理者ユーザーログインが `admin_user` 表に示されます。 [GitHub-36642](https://github.com/magento/magento2/issues/36642)

<!-- AC-7507-->

* テーブル作成に関する情報が、 `Authorization/README.md`. [GitHub-36644](https://github.com/magento/magento2/issues/36644)

<!-- AC-7511-->

* プロジェクトの行動規範 (`CODE_OF_CONDUCT.md`) をクリックします。 [GitHub-36639](https://github.com/magento/magento2/issues/36639)

<!-- AC-7500-->

* Google Adwords コードテンプレートは、読みやすさを向上させるためにリファクタリングされ、非推奨のエスケープ関数を正しい関数に置き換えました。 [GitHub-36641](https://github.com/magento/magento2/issues/36641)
...
* この `Fixex.io` インポートサービスが期待どおりに動作するようになりました。 通貨レートが正常にインポートされるようになりました。 [GitHub-36010](https://github.com/magento/partners-magento2ee/issues/36010)
...
* 見つからない `scope` 値が `isShippingCarrierAvailable`. [GitHub-36645](https://github.com/magento/magento2/issues/36645)

<!-- AC-7503-->

* これで、買い物かごページから提供された送料の推定が正確になりました。 collectShippingRates が 1 回だけ設定されるようになりました。 以前は、送料の見積もりが複製されていました。 [GitHub-36648](https://github.com/magento/magento2/issues/36648)
...
* チェコ共和国向けの ISO 3166 地域が追加されました。 `directory_country_region` 表。 [GitHub-36346](https://github.com/magento/magento2/issues/36346)
...
* 0 より大きく、1 より小さい品目数量を含む注文に対して、出荷を作成できるようになりました。 **[!UICONTROL Quantity uses decimals]** が有効になっている。 以前は、1 未満の製品数量を入力した場合、請求書は正しく作成されましたが、出荷は作成されませんでした。 [GitHub-35971](https://github.com/magento/magento2/issues/35971)

<!-- AC-7510-->

* GitHub の問題テンプレートは、開発者向けの問題や機能のリクエストを考慮して、フォームに変換されました。[GitHub-36651](https://github.com/magento/magento2/issues/36651)

<!-- AC-7509-->

* 冗長 `getCacheTags` メソッドが `Topmenu` クラス。 [GitHub-36643](https://github.com/magento/magento2/issues/36643)

<!-- AC-7504-->

* 住所送料のモーダルのスタイルを設定するクラスを追加しました。 この特定のクラスがないと、フロントエンド開発者はチェックアウトワークフローの他のモデルに影響を与えずに、モーダル全体のスタイルを設定できません。 [GitHub-36647](https://github.com/magento/magento2/issues/36647)

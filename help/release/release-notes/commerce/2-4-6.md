...
このリリースには、8 つのセキュリティ修正とプラットフォームのセキュリティ強化が含まれています。 これらのセキュリティ修正は、Adobe Commerce 2.4.5-p1 およびAdobe Commerce 2.4.4-p3 に移植されました。
...
* MariaDB 10.6 （LST バージョン）。 このリリースは MariaDB 10.4 と互換性がありますが、MariaDB 10.6 にアップグレードすることをお勧めします。 <!-- AC-6921 -->
...
* 実行中 `bin/magento set:di:com -vvv` ( または `-v` または `-vv` オプション ) が失敗した場合に関する情報メッセージを返すようになりました。 [GitHub-35391](https://github.com/magento/magento2/issues/35391)
...
* この `composer install` 本体が上書きされなくなりました `.gitignore` ファイル。 [GitHub-32888](https://github.com/magento/magento2/issues/32888)

<!-- AC-6671-->

* 実行中 `php bin/magento staticdata:remove` アップグレード後にサンプルデータを削除するには、このエラーで失敗しなくなりました。 `Undefined constant "MagentoHackathon\Composer\Magento\Deploystrategy\GLOB_BRACE`. [GitHub-36062](https://github.com/magento/magento2/issues/36062)
...
* この `bin/magento app:config:import`コマンドで、 `config.php` ファイル。 以前は、次のエラーでインポートに失敗していました： `Import failed: Notice: Trying to access array offset on value of type null in [..]/vendor/magento/module-config/Model/Config/Backend/Currency/Cron.php on line 66`. [GitHub-31428](https://github.com/magento/magento2/issues/31428)
...
* reCAPTCHA が有効な場合に、Braintreeを使用して注文を配置する際に、買い物客が reCAPTCHA 検証をバイパスできなくなりました。

<!-- BUNDLE-3185-->

* 複数の宛先に出荷された注文のチェックアウト時の ReCAPTCHA 検証が、期待どおりに動作するようになりました。
...
* SEO に適した URL が、 `url_key` 属性は、store-view レベルで上書きされます。

...
修正された問題の最新情報については、[Adobeセキュリティ速報 ](https://helpx.adobe.com/security/products/magento/apsb23-17.html) を参照してください。
...
* Elasticsearch 8.x[ アップグレードElasticsearch](../../../upgrade/prepare/prerequisites.md#upgrade-elasticsearch) を参照してください。<!-- AC-6597 -->
...
* 提供終了に達した Zend フレームワーク （ZF1） コンポーネントは、コードベースから削除されました。 [ 後方互換性のない変更のハイライト ](https://developer-stage.adobe.com/commerce/php/development/backward-incompatible-changes/highlights/) を参照してください。<!-- AC-1512 -->
...
* [Adobe CommerceのAdobe I/O イベント ](https://developer.adobe.com/commerce/events/get-started/)

この拡張機能メタパッケージの今後のバージョンには、追加の拡張機能が含まれます。

### Adobe CommerceのAdobe I/Oイベント

Adobe CommerceのAdobe I/Oイベントは、Commerce イベントをキャプチャし、Adobe Commerceから他のAdobeシステムにデータを転送します。 開発者は、在庫の更新、注文ステータスの変更、価格の更新などのCommerce イベントにサブスクライブする、クラウドネイティブなアプリをAdobe Developer App Builderで作成できます。 開発者は、この機能を使用して、ミッションクリティカルなシステム全体でバイヤージャーニー全体のオーケストレーションのデータを交換できます。
...
* イタリアとスペインの **[!UICONTROL Pay Later]** ボタンとバナー（メッセージ）のBraintreeが有効になりました。
...
* 不正防止と ACH Web フックを追加しました。 Fraud Protection Web フックは、Braintreeでリスクの判断が行われた際にトリガーされます。 ACH Web フックは、ACH 支払いのステータスが `settled` または `settlement decline` に更新されたときにトリガーされます。
...
**顧客クエリで順序の並べ替えがサポートされるようになりました**。 `customer` クエリの `orders` 出力オブジェクトに、並べ替えフィールドが含まれるようになりました。 オーダーをオーダー番号または作成日で、昇順または降順で並べ替えられるようになりました。<!-- AC-6903-->
...
PWA Studio v.13.0.x はAdobe Commerce 2.4.6 と互換性があります。アクセシビリティを向上させるための複数の機能強化が含まれています。 バグの修正については、[PWA Studioリリース ](https://github.com/magento/pwa-studio/releases) を参照してください。 PWA Studioバージョンと互換性のあるAdobe Commerce コアバージョンのリストについては、[ バージョンの互換性 ](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) を参照してください。
...
* 製品の `sales_order_item` 表の `discount_percent` 列に、品目割引数量が設定されていない場合、または受注品目数量以上の場合に、割引率が期待どおりに入力されるようになりました。 以前は、この列は、この商品に買い物かごの価格ルールが適用された際に入力されていませんでした。
...
* クーポンの生成時にクーポンコード設定を適用する検証ロジックがクーポンコードの管理ワークフローに追加されました。 以前は、**クーポン数** の値が 2500 より大きい場合、クーポンコードの生成で **[!UICONTROL Code Length]** 設定が無視されていました。
...
* **EU 諸国** 設定（**[!UICONTROL Stores]**/**[!UICONTROL Configuration]**/**[!UICONTROL General]**/**[!UICONTROL Country options]**/**[!UICONTROL European Union Countries]**）に国がない場合、管理者ユーザーは Admin **[!UICONTROL Store]**/**[!UICONTROL Configuration]** ページを正常に開くことができるようになりました。 以前は、管理者ユーザーがこのページを開こうとすると、Adobe Commerceが次のエラーをスローしていました：`TypeError: explode(): Argument #2 ($string) must be of type string, null given in /var/www/php/folder/vendor/magento/module-config/ViewModel/CountryFilter.php:46`。 [GitHub-35901](https://github.com/magento/magento2/issues/35901)
...
* 子カテゴリを持つカテゴリのカテゴリ `urlKey` を保存する際に、正しい `rowId` パラメーターが渡されるようになりました。 以前は、カテゴリ `URLKey` を保存する際に、間違った `categoryId` が渡されていました。
...
* **[!UICONTROL Enable Hierarchy Functionality]** を無効にすると、CMS ページ階層のパンくずリストは表示されなくなります。 以前は、パンくずリストは表示されていましたが、買い物客がクリックすると、404 ページが表示されていました。 [GitHub-35513](https://github.com/magento/magento2/issues/35513)
...
* `recordLogin` メソッドが更新され、ユーザーモデルにログイン値が設定され、`admin_user` テーブルの `logdate` フィールドと `lognum` フィールドに正しい値が保持されるようになりました。 以前は、更新された管理者ユーザーのログイン試行は記録されていませんでした。
...
* **[!UICONTROL Use in Filter Option]s** 設定が無効の場合、顧客および顧客の住所属性の **[!UICONTROL Grid Filter Condition Type]** フィールドが属性編集ページに表示されなくなりました。
...
* **[!UICONTROL My Quotes]** ページの **[!UICONTROL Show my quotes]** フィルターが期待どおりに動作するようになりました。 以前は、フィルター結果には他の管理者ユーザーの引用符が含まれていました。
...
* GraphQL コードで複数のインターフェイスを実装する `Types` ールは、GraphQL `@doc` 仕様に従って、`&` 記号で区切 `,` ことも、区切ることもできます。
...
* `product` クエリは、正しいカテゴリデータを集計で返すようになりました。 以前は、`attribute_code.category_uid` は `category_uid` ではなく `category_id` の値を返していました。
...
* `bin/magento app:config:import` コマンドは、特定の `cron` 式が含まれている場合に、`config.php` ファイルの内容を正常に読み込むようになりました。 以前は、読み込みは次のエラーで失敗していました：`Import failed: Notice: Trying to access array offset on value of type null in [..]/vendor/magento/module-config/Model/Config/Backend/Currency/Cron.php on line 66`。 [GitHub-31428](https://github.com/magento/magento2/issues/31428)
...
* `catalogpermissions_category` インデックスのインデクサー `batch_size` が、期待どおりに、`env.php` ファイルで指定された値に変更されました。 以前は、インデクサー `batch_size` のパスの誤入力により、バッチサイズを `env.php` で指定された値に変更できませんでした。 [GitHub-652](https://github.com/magento/partners-magento2ee/issues/652)
...
* `escaper.js` `escapeHtml` 関数は、アンパサンド （&amp;）特殊文字を `InnerHtml` ルールに従って `"$amp;"` に変換するようになりました。 この一般的な解決策により、タイトル入力ボックスの特殊文字および文字参照の変換に関する問題が解決され、挿入されたとおりの正確な値が保存されます。 以前は、アンパサンド（&amp;）文字は、コンテンツ階層のノード名で誤ってエンコードされていました。
...
* 会社属性値が、チェックアウトワークフローの請求セクションと配送先住所セクションに期待どおりに表示されるようになりました。 以前は、注文作成時に店舗が正しく設定されていなかったので、会社属性が注文詳細ページに正しく表示されませんでした。 属性値が正しく格納されるのは、マルチストアデプロイメントで **メイン Web サイト** の **会社を表示** プロパティが **[!UICONTROL optional]** に設定されている場合のみです。
...
* 管理者で Vault がオフになっている場合、`customerPaymentTokens` クエリはBraintreeトークンを返さなくなりました。
...
* BraintreePayPal が有効で **[!UICONTROL Enable Card Payments]** が無効の場合、Braintree支払い方法を使用したチェックアウトが正常に完了するようになりました。 以前は、進行状況バーが解決されず、Adobe Commerceに「`adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66)`」というコンソールエラーが表示されていました。 [GitHub-35961](https://github.com/magento/magento2/issues/35961)

<!-- BUNDLE-3140 3141-->

* 不正防止、ACH 支払い、ローカル支払い方法の Web フックが実装されました。 不正防止 Web フック リスクの判断がBraintreeされた場合にのみトリガーされます。 ACH Web フックは、ACH 支払いが `settled` または `settlement decline` に更新された後にのみトリガーされます。
...
* 買い物客が請求先住所を更新しても、チェックアウトワークフローでは、「Braintreeコンテナの支払い方法」オプションが選択されたままになります。 以前は、買い物客が「**[!UICONTROL My billing and shipping address are the same]**」ボタンの選択を解除すると、支払い方法の選択が解除されていました。
...
* 管理者から Vault がオフになっている場合、`customerPaymentTokens` クエリはBraintreeトークンを返さなくなりました。
...
* 買い物客は、**[!UICONTROL Enable Card Payments]** が無効になっているストアフロントでBraintree支払い方法を使用できるようになりました。 以前は、Adobe Commerceは、次のJavaScript エラーをスローしていました。`_adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66).`
...
* `Braintree/Ui/Component/Report/Listing/Column/PaymentType.php` の `ANDROID_PAY_CARD` 定数は `GOOGLE_PAY_CARD` に置き換えられました。 これにより、Braintreeレポートの問題が解決されます。
...
* **[!UICONTROL Enable RMA]** フィールドの「**[!UICONTROL Use Config Settings]**」チェックボックスで、製品の保存時に期待どおりに値が保存されるようになりました。
...
* `setShippingMethodsOnCart` ミューテーションの出力に `price_excl_tax` タイプと `price_excl_tax` タイプを追加しました。 [GitHub-31206](https://github.com/magento/magento2/issues/31206)
...
* 日本語の文字を製品ページ属性セット（コンボボックス）に直接入力できるようになりました。 以前は、このコンボボックスに直接日本語を入力すると、MacOSで Firefox ブラウザーを実行する際に、文字が変更または削除されていました。 すべてのブラウザーで日本語HTMLの処理が改善される `textInput` 文字の属性が、`Value` 属性と `valueUpdate` 属性に置き換えられました。
...
* Admin **[!UICONTROL Stores]** の `Search Weight` 属性の `Storefront Properties` 入力フィールドの下に情報テキストを追加しました  > [!UICONTROL Attributes] > **[!UICONTROL Product]**。
...
3. このパッチは、アップグレード中、パッケージの管理フェーズの後、メタデータの更新フェーズの前に適用します。詳しくは、[Adobeが提供するコンポーザーパッチの適用方法 ](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/how-to/how-to-apply-a-composer-patch-provided-by-magento.html) を参照してください。
4. アップグレードプロセスを完了します。

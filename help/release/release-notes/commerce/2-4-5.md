...
Adobe Commerceでは、プラットフォームの品質、支払い方法、GraphQL キャッシュのパフォーマンス、アクセシビリティの改善が導入されました。 統合されたGoogleモジュールとアップグレード互換性ツールのアップデートが含まれ、ライブ検索 B2B のサポートが導入されています。
...
* GraphQL 要求の後、Adobe Commerceセッションは終了しなくなりました。 以前は、 `ClearCustomerSessionAfterRequest` プラグインは買い物客にログアウトしました。 [GitHub-34550](https://github.com/magento/magento2/issues/34550)
...
**問題**:Adobe Commerce 2.4.5 の新規インストール（アップグレードではない）後、RabbitMQ のインフラストラクチャ要素（交換、キュー）は初期化されない場合があります。この誤った設定の RabbitMQ は、非同期操作が実行されたり、パフォーマンスプロファイルが生成されたりすると致命的なエラーを引き起こします。 クラウドのデプロイメントには影響はありません。 **回避策**:オンプレミスでAdobe Commerce 2.4.5 の新規インストールを実行するマーチャントは、 `bin/magento setup:upgrade --keep-generated` 使用後 `bin/magento setup:config:set` RabbitMQ 設定オプションをに渡して RabbitMQ を設定するかAdobe Commerceをインストールするには `setup:install` コマンドを使用します。 詳しくは、 [RabbitMQ](../../../installation/prerequisites/rabbitmq.md).

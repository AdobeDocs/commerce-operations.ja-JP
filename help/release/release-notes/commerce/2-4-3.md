...
* GraphQL `cart` クエリで、クーポンが注文に適用された場合に、買い物かごの請求ステップに対する正しい総計が返されるようになりました。
...
* カスタム URL を使用して管理者から作成された URL リダイレクトが正常に機能するようになりました。 以前は、GraphQL は GraphQL からの応答をキャッシュしていました `urlResolver` クエリを実行し、URL の書き換えの更新後に古い値を返す。
...
**問題**:GraphQL `category` および `categoryList` クエリは、共有カタログのカテゴリを非表示にするか公開するかを決定するカテゴリ権限を無視します。 ストアフロントには、現在、共有カタログに割り当て済みまたは割り当て解除済みのすべてのカテゴリが表示されます。 これは、B2B 共有カタログが有効になっているPWAストアフロントを実装するAdobe Commerce 2.4.3 のデプロイメントに影響します。 <!-- MC-42528-->

...
[2}Adobe Sensei](https://www.adobe.com/sensei.html) を活用した ](https://experienceleague.adobe.com/en/docs/commerce-merchant-services/live-search/overview) ライブサーチ } では、人工知能と機械学習アルゴリズムを使用して、集計した訪問者データの詳細な分析を行うことで、直感的な検索エクスペリエンスを提供します。[ [Live Search リリースノート ](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/release-notes.html) を参照してください。
...
* **ウィッシュリスト**。 [addWishlistItemsToCart mutation](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-wishlist-items-to-cart.html) は、指定したウィッシュリストから顧客の買い物かごに項目を移動します。<!-- magento/partners-magento2ee#449-->

* **ギフトレジストリ**。 以下のタスクが対象となります。<!-- PWA-1290-->

   * [giftRegistry クエリ ](https://developer.adobe.com/commerce/webapi/graphql/queries/gift-registry.html) を使用して、顧客のギフトレジストリの内容を返します。

   * [ タイプ ](https://developer.adobe.com/commerce/webapi/graphql/queries/gift-registry-type-search.html)、[ 電子メール ](https://developer.adobe.com/commerce/webapi/graphql/queries/gift-registry-email-search.html)、または [ID](https://developer.adobe.com/commerce/webapi/graphql/queries/gift-registry-id-search.html) でギフト レジストリを検索します。

   * ギフト レジストリを [ 作成 ](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-gift-registry.html)、[ 更新 ](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-gift-registry.html)、または [ 削除 ](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-gift-registry.html) します。

   * ギフト レジストリのアイテムを [ 更新 ](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-gift-registry-items.html) または [ 削除 ](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-gift-registry-items.html) します。

   * [ すべてのアイテムを移動 ](https://developer.adobe.com/commerce/webapi/graphql/mutations/move-cart-items-to-gift-registry.html) カートからギフト レジストリへ。<!-- ENGCOM-9082 9087-->

   * ギフトレジストリから登録者を [ 追加 ](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-gift-registry-registrants.html)、[ 更新 ](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-gift-registry-registrants.html)、または [ 削除 ](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-gift-registry-registrants.html) します。

   * [ 共有 ](https://developer.adobe.com/commerce/webapi/graphql/mutations/share-gift-registry.html) 招待者付きギフト レジストリ。

* **交渉可能な見積もり**. [`negotiableQuote`](https://developer.adobe.com/commerce/webapi/graphql/queries/negotiable-quote.html) クエリと [`negotiableQuotes`](https://developer.adobe.com/commerce/webapi/graphql/queries/negotiable-quotes.html) クエリを参照してください。<!-- PWA-1292-->
...
* **共有ルート**。 [route query](https://developer.adobe.com/commerce/webapi/graphql/queries/route.html) および [RoutableInterface](https://developer.adobe.com/commerce/webapi/graphql/interfaces/routable-interface.html) は、product、category、およびCMS ページ上のルーティングリクエストをサポートします。 `urlResolver` クエリは非推奨（廃止予定）になり、その機能は `route` クエリに置き換えられました。
...
* カスタム権限を持つ管理者の場合、管理 GWS が管理コレクションの `website_id` SQL 条件に `int` の値を使用するようになりました。
...
* REST/SOAP API 用のカスタマーセグメントイベントオブザーバーを追加しました。 その結果、REST/SOAP API を通じて注文が行われる際に、セグメントで定義された条件が一致した場合、お客様のセグメントが期待どおりに自動的に更新されるようになりました。
...
* `bin/magento setup:db-schema:split-sales` を使用したデータベーステーブルの移動が期待どおりに動作するようになりました。 テーブルが作成され、データが正しく入力されるようになりました。 以前は、テーブルの存在のチェックが正しくなかったので、`bin/magento setup:db-schema:split-sales` を実行するとアプリケーションがエラー `SQLSTATE` スローしていました。
...
* Google Analyticsが有効になっている場合に、管理者がストアフロントでページを読み込もうとしても、アプリケーションがエラーをスローしなくなりました。 AJAX リクエスト to www.google-analytics.comを許可するように `connect-src` の CSP エントリが追加されました。 [GitHub-30880](https://github.com/magento/magento2/issues/30880)
...
* [`dynamicBlocks` クエリは ](https://developer.adobe.com/commerce/webapi/graphql/queries/dynamic-blocks.html) 指定したフィルターに一致する動的ブロックのコンテンツを返します。
...
* 属性フィールドの値 `required` が **no** に設定されている場合、RMA `resolution` 属性は必要に応じて表示されなくなります。
...
* `Magento\Elasticsearch\Model\Adapter\FieldMapper\Product\FieldProvider\FieldName\Resolver\CompositeResolver` にカスタムElasticsearchフィールドマッパーを追加できるようになりました。 [GitHub-24350](https://github.com/magento/magento2/issues/24350)
...
* `customer/create_account/vat_frontend_visibility` が有効な場合、顧客登録ページに **VAT 番号** フィールドが表示されるようになりました。 [GitHub-31326](https://github.com/magento/magento2/issues/31326)
...
* インライン翻訳が設定されている場合、文字列に追加される角括弧がエスケープされなくなりました。 以前は、`escapeHtmlAttr` メソッドは、括弧をHTML図形コードに変換していました。 [GitHub-32000](https://github.com/magento/magento2/issues/32000)

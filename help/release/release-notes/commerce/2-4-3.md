...
このホットフィックスは、配列に 20 個を超える項目を含むリクエストを web API で処理できない問題の解決策を提供します。 この問題は、Magento Open Source 2.4.3、Adobe Commerce 2.4.3、または 2.3.7-p1 を実行するデプロイメントに影響します。 サービス拒否（DoS）攻撃を防ぐために、これらのリリースには組み込みのレート制限が追加され、デフォルトの最大値は 20 に設定されていました。 このパッチは、デフォルトの制限を高い値に戻します。 ストアで DoS 攻撃が発生している疑いがある場合は、デフォルトの入力制限を低い値に下げてAdobe Systems、要求できるリソースの数を制限することをお勧めします。 ナレッジベース記事「 [Web API は配列に 20 を超える項目を含むリクエストを処理できない](https://support.magento.com/hc/en-us/articles/4406893342093) を参照してください。
...
このエラーは、PHP 8.x 関数である `str_contains` 関数の使用が原因で発生します。 Adobe Systems Commerce 2.4.3 は PHP 8.x をサポートしていません。この修正プログラムは、サポートされている PHP 7.x 関数でこの関数を置き換えます。 PHP の致命的なエラー Hotfix[&#128279;](https://support.magento.com/hc/en-us/articles/4408021533069-Adobe-Commerce-upgrade-2-4-3-2-3-7-p1-PHP-Fatal-error-Hotfix)ナレッジベース記事Adobe Systems Commerce へのアップグレード 2.4.3、2.3.7-p1 を参照してください。
...
* Laminas ライブラリの依存関係は、PHP 8.x 互換バージョンにアップグレードされました。 一部の冗長な依存関係が `composer.json` ファイルから削除されました。 **Adobe Systems Commerce 2.4.3 は Laminas 3.4.0** を使用します。 <!-- MC-39513-->

### パフォーマンスの強化

このリリースには、製品価格とカタログ ルールのインデクサーのインデックス作成時間を短縮する機能強化が含まれています。 マーチャントは、顧客グループまたは共有カタログからWebサイトを除外できるようになり、インデックス作成のレコード数が減り、インデックス作成時間が短縮されます。

### ライブ サーチです

[Live Search](https://experienceleague.adobe.com/en/docs/commerce/live-search/overview) [Adobe Sensei](https://www.adobe.com/sensei.html) は、人工インテリジェンスと機械学習アルゴリズムを使用して集約された訪問者データの詳細な分析を実行することで、直感的な検索エクスペリエンスを提供します。 [Live Search リリースノート ](https://experienceleague.adobe.com/docs/commerce/live-search/release-notes.html) を参照してください。
...
バージョン 2.4.3 では、B2B v1.3.2 が導入されています。このリリースには、複数のバグ修正が含まれています。 [B2B リリースノート ](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html) を参照してください。
...
[ アップグレード互換性ツール ](https://experienceleague.adobe.com/docs/commerce-operations/upgrade-guide/upgrade-compatibility-tool/overview.html) の範囲を、コミュニティからのフィードバックに基づいて拡大しました。 アドビの [#upgrade-compatibility-tool](https://magentocommeng.slack.com/archives/C019Y143U9F) Slack チャンネルに参加して、Adobe製品チームやコミュニティのサポートを得たり、ツールの今後の方向性をガイドしたりしてください。

### Cloud Managed Services のアップデート

このリリースには、Amazon シンプルストレージサービス (AWS S3) および Amazon Aurora クラウド マネージドサービスのサポートの機能強化が含まれています。 AWS ElastiCache、AWS ElasticSearch、および AWS Managed Queues (Rabbit MQ) の認定サポートを提供します。 (これらのサービスの機能、パフォーマンス、および Adobe Systems Commerce との統合をテストしました)。
...
* [Amazon支払 ](https://marketplace.magento.com/amzn-amazon-pay-magento-2-module.html)。 Amazon Pay は非推奨（廃止予定）となり、2.4.x の今後のリリースで削除される予定です。 バージョン 2.4.3 以降には、互換性のアップデートと主要なバグの修正のみが含まれます。

* [ブレーントリー](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/payments/braintree)

* [dotdigital エンゲージメントクラウド](../../../upgrade/modules/upgrade.md)
...
* [ヨッポ商品レビュー](../../../upgrade/modules/upgrade.md)
...
* Grantまたはサーバーサイドコンパイルでより少ないファイルをコンパイルすると、同じ結果が得られるようになりました。 先に。 `.abs- styles`は、`_extends.less` の他の`.abs- styles`を拡張したもので、Grunt でコンパイルした場合に正しく出力されませんでした。その結果、実稼動デプロイメントと開発デプロイメントの違いが生じました。 [GitHub-7231](https://github.com/magento/magento2/issues/7231)

### AdminGWS

<!-- MC-40703-->

* 管理者 GWS は、カスタム権限を持つ管理者の管理者コレクションの `website_id` SQL 条件に`int`値を使用するようになりました。
...
* アプリケーションは、買い物かご内の製品量を変換および更新するときに、アカウントロケール固有の小数点ロケーターを取り込むようになりました。

<!-- MC-41574-->

* 注文で、指定された顧客名と配送先住所が省略されなくなりました。 以前は、 `same_as_billing` フラグがデータベースに保存されなかったため、名前は省略されていました。

<!-- MC-40946-->

* ギフトレジストリへのリンクは、買い物かごで商品を編集するときに期待どおりに保持されるようになりました。 以前は、「**買い物かごを更新** ボタンをクリックすると、これらのリンクは表示されていませんでした。

<!-- MC-40696-->

* コンシューマー `quoteItemCleaner` すべてを選択キューメッセージは、いくつかの製品の削除後に期待どおりにステータスが [ `complete` ] に変更されます。 以前は、このコンシューマーの 1 つのメッセージのみがステータスを `complete` に変更し、残りのメッセージはステータスを `in progress` に変更していました。
...
* 特例文字テーマのレイアウト更新が期待どおりに適用されるようになりました。 以前は、カスタムテーマレイアウトの更新は無視されていました。

<!-- MC-41499-->

* 製品カテゴリキャッシュは、`indexer_update_all_views`実行中に `cron` によって期待どおりにクリアされるようになりました。以前は、インデックス再作成後のカテゴリーページの製品数が正しくありませんでした。
...
* 特例文字カテゴリレイアウト更新ファイルが期待どおりに製品に適用されるようになりました。 以前は、更新ファイルハンドル (`catalog_category_view_*`) が製品ハンドルと一致しませんでした。 [GitHub-27285](https://github.com/magento/magento2/issues/27285)

<!-- MC-40679-->

* 管理商品リストの固定商品税(FPT)列の並べ替えが無効になっています。 以前は、FPT 列を並べ替えた後、製品 ページをリロードできませんでした。
...
* GET `/rest/V1/products/?searchCriteria[filterGroups]` を使用した商品検索上の位置による並べ替えが、期待どおりに機能するようになりました。 以前は、製品コレクションには並べ替え用のフィールド `position` 値がありませんでした。 [GitHub-31591](https://github.com/magento/magento2/issues/31591)

<!-- MC-40122-->

* 管理者ユーザーは、商品グリッドの **名前** フィールドと **SKU** フィールドに重複スペースを表示できるようになりました。 以前は、複数のスペースを 1 つのスペースに折りたたんでいました。

<!-- MC-39869-->

* 管理者での販売可能量が 0 の場合、製品がストアフロントに在庫切れとして表示されるようになりました。 以前は、これらの商品は店頭に在庫ありとしてリストされており、アプリケーションには有効な **追加から買い物かご** ボタンが表示されていました。 [GitHub-31117](https://github.com/magento/magento2/issues/31117)

<!-- MC-39809-->

* 管理者は、管理顧客ページの [ショッピング カート] セクション ([顧客の活動] 列) から [注文された品目] グリッドに `(File)` カスタマイズ可能なオプションを持つ製品を追加できるようになりました。 以前は、値が `\Magento\Catalog\Model\Product\Type\AbstractType::_prepareOptions`に挿入される前に正しく書式設定されていなかったため、アプリケーションはアイテムをリストに追加しませんでした。
...
* Adobe Systems Commerce は、管理関連製品、アップセル、およびクロス販売リストページあたりの値を変更すると、期待どおりに合計閲覧ページ数を更新します。 [GitHub-31059](https://github.com/magento/magento2/issues/31059)

<!-- MC-39444-->

* 管理者は、SKUによる注文に `(File)` 2 つ以上のカスタマイズ可能なオプションを持つ製品を追加できるようになりました。

<!-- MC-39140-->

* 権限が制限された管理者が管理のCMSページに製品ウィジェットを追加したときに、アプリケーションがエラーをスローしなくなりました。 以前は、管理者が **保存** ボタン:  `We are sorry, an error has occurred while generating the content` をクリックすると、アプリケーションはこのエラーをスローしていました。

<!-- MC-39134-->

* **JavaScriptカレンダーを使用**&#x200B;が有効になっている場合に、前回の注文の日付が入力された日付のカスタマイズ可能な日付で商品詳細ページが開くようになりました。カスタム日付オプション値リゾルバーは、値が現在の構成に基づいてフォーマットされていない場合、代替形式にフォールバックするようになりました。 以前は、カスタム日付オプションの値は空でした。

<!-- MC-39104-->

* 商品が在庫切れの場合、買い物かごアプリケーションに 1 つのエラーのみが表示されるようになりました。 以前は、アプリケーションに冗長なメッセージが表示されていました。 [GitHub-27469](https://github.com/magento/magento2/issues/27469)

<!-- MC-35717  magento/partners-magento2ee#427-->

* 管理者は、SKUによる注文に `(File)` カスタマイズ可能なオプションを備えた製品を追加できるようになりました。 [GitHub-30285](https://github.com/magento/magento2/issues/30285)
...
* REST `PUT /V1/products/:sku/links` リクエストを使用して新しい子製品を新しいグループ製品に関連付けると、グループ製品が期待どおりにストアフロントで使用できるようになりました。 以前は、 `bin/magento cron:run`を実行した後に製品のインデックスが正しく作成されませんでした。

<!-- ENGCOM-9010 8947-->

* `NULL`SKU値を持つ製品は作成できなくなりました。以前は、カスタムインポーターを使用して、またはデータベースで直接、SKU値なしで製品を作成できましたが、管理者から編集しようとすると、アプリケーションエラーがスローされました。 [GitHub-27411](https://github.com/magento/magento2/issues/27411)、 [GitHub-32525](https://github.com/magento/magento2/issues/32525)

<!-- ENGCOM-8902-->

* 単純な製品に必要なカスタム オプションを追加しても、警告なしに親複合製品から削除されることがなくなりました。 アプリケーションに情報警告が表示され、商品が保存されなくなるようになりました。  以前は、アプリケーションは製品の変更を保存し、警告を表示しませんでした。 [GitHub-30492](https://github.com/magento/magento2/issues/30492)
...
* 大画像は、 **フロントエンドのサイズ変更を有効にする** 構成設定が有効になっているアップロード中に期待どおりにサイズが変更されるようになりました。
...
* コンテンツセキュリティポリシーで、`data: scheme` を使用した base64 エンコードされた画像およびフォントの読み込みがサポートされるようになりました。

### cron

<!-- MC-30804-->

* Cronクリーンアップクエリは、次のパフォーマンスの問題を軽減または排除するためにリファクタリングされました: `cron` ジョブが保留中の状態でスタックしたまま、MySQLクエリがますます遅くなり、CPU使用率が増加しています。 [GitHub-26507](https://github.com/magento/magento2/issues/26507)
...
* REST GET買い物かご API 呼び出しで、請求および配送先住所に対して正しいカスタム属性値が返されるようになりました。 以前は、マイアカウントストアフロントページと管理の注文の詳細ページにカスタム住所属性が正しく表示されませんでした。

<!-- MC-24495-->

* これでアプリケーションは、入力タイプ `file (attachment)` の 顧客 アドレス属性を含むファイルを正常に処理できるようになりました。 以前は、アプリケーションは添付ファイルアップロード中にこのエラーをスローしました: `Something went wrong while saving the file`。

### 顧客

<!-- MC-41216-->

* ストアクレジットメールは、選択した範囲アカウント取り込み、正しいメールアドレスから送信されるようになりました。
...
* 顧客 グリッド フィルターでは、データが以前にキャッシュされていた場合、制限付きユーザーに対して正しい Web サイト オプションが使用されるようになりました。 以前は、顧客グリッドフィルターは、キャッシュから web サイトパラメーターを取得し、制限されたユーザーに対して正しくないデータを含めていました。

<!-- MC-39189-->

* アプリケーションでは、複数の Web サイトデプロイメントで 1 つの Web サイトが削除されたときに、管理者顧客ページに例外がスローされなくなりました。 以前は、管理者が包括的な顧客リストにアクセスしようとすると、アプリケーションにすべての顧客が表示されず、次のエラーが表示されました: `The website with id 2 that was requested wasn't found. Verify the website and try again`。

<!-- MC-38913-->

* 権限を持つ管理者は、顧客のアカウント情報タブから別のWebサイトに顧客を再割り当てできるようになりました。

<!-- MC-39852-->

* 入力タイプが `(File)` の顧客住所属性を作成するときに、ファイルを正常にアップロードできるようになりました。 以前は、ファイルをアップロードして保存しようとすると、アプリケーションが次のエラーをスローしていました：`Something went wrong while saving the file`。
...
* 顧客セグメントは、管理者から注文があると、自動的に更新されるようになりました。 以前は、管理注文を作成した後、管理ユーザーは、関連する顧客セグメントに移動し、 **更新セグメントデータ** ボタンをクリックして、関連するセグメントデータを手動で更新する必要がありました。

<!-- MC-39490-->

* このアプリケーションでは、関連する顧客セグメント内のすべての顧客について、買い物かごに動的ブロックが表示されるようになりました。 以前は、顧客セグメントにゲストと登録済み顧客の両方が含まれている場合、ゲストにはブロック均等が表示されませんでした。
...
* 顧客が [お問い合わせ] フォームから電子メールを送信しようとしたときに例外が発生した場合に、アプリケーション が期待どおりにエラーを記録するようになりました。 [GitHub-23645](https://github.com/magento/magento2/issues/23645)

### フレームワーク

<!-- MC-41712-->

* Admin の親クラスは、元のクラスと継承されたインターフェイスと共に docblock 注釈チェックされるようになりました。 以前は、親クラスがチェックされていなかったため、イベントをトリガーした Admin で顧客関連のアクションを実行するとエラーが発生していました。 アプリケーションは、このエラーを例外ログに記録しました: `report.CRITICAL: Method's return type must be specified using @return annotation.`。

<!-- MC-41192-->

* Redis ページ キャッシュが有効になっているデプロイメントで Redis サーバーが停止したときに、アプリケーションが致命的なエラーをスローしなくなりました。

<!-- MC-40739-->

* バックアップで`bin/magento config:set system/backup/functionality_enabled 1`が有効になっている場合、アプリケーションは&#x200B;**フォルダーをバックアップから除外メディア**&#x200B;設定に従うようになりました。以前は、この設定にもかかわらず、`/magento` へのパスが二重の//で正しく形成されていなかったので、メディアフォルダーがバックアップされていました。

<!-- MC-40654 ENGCOM-8881-->

* アプリケーション は、Admin 買い物かご ページの期待どおりにすべての翻訳可能な文字列を翻訳するようになりました。 以前は、変換の読み込みが遅すぎて、コントローラーアクションの事前ディスパッチにサブスクライブしていたすべてのオブザーバーがスキップされていました。 [GitHub-31849](https://github.com/magento/magento2/issues/31849)

<!-- MC-39776-->

* 新規作成顧客アカウントのフォーム検証ページ **ログイン** 拡張機能を有効にする設定が無効になっている場合、正常に機能するようになりました。 以前は、アプリケーションはJavaScriptエラーをスローしていました。

<!-- MC-39537-->

* `bin/magento setup:db-schema:split-sales`を使用したデータベーステーブルの移動が、期待どおりに動作するようになりました。これでTablesが正しく作成され、データが正しく入力されるようになりました。 以前は、テーブルの存在の確認が正しくなかったため、`bin/magento setup:db-schema:split-sales`を実行するとアプリケーション`SQLSTATE`エラーがスローされました。
...
* 初期化中に発生した例外はキャッシュされなくなり、500 応答コードをトリガーするようになりました。

<!-- MC-33179  magento/partners-magento2ee#431-->

* マーチャントは、ストア表示ごとにカタログイベントに対して画像を割り当てたり、カタログから削除したりできるようになりました。 以前は、カタログイベントの編集中にストア表示を変更すると、&lbrack; **削除画像** チェックボックスがアクティブではなく、アプリケーションに次のエラーが表示されました。 `Uncaught ReferenceError: toggleValueElements is not defined at HTMLInputElement.onclick`。 [GitHub-364](https://github.com/magento/partners-magento2ee/issues/364)

<!-- ENGCOM-8232-->

* `communication.xml` ファイルにマージされた最後のハンドラーは、以前に作成されたすべてのハンドラーをオーバーライドしなくなりました。[GitHub-29528](https://github.com/magento/magento2/issues/29528)

### 一般修正点

<!-- MC-41194-->

* アプリケーションでは、[マイ プロダクト レビュー] ページと [マイ アカウント] ページ最近のレビューに正しい数の星が表示されるようになりました。 以前は、アプリケーションは最初のレビューにのみレビュースターを適用し、ページの他のレビューにはスターを付けずに残していました。

<!-- MC-40653-->

* ヘッダー内のアカウントリンクが WCAG 標準フォローするようになりました。 以前は、ヘッダー内のアカウントリンクに重複した ID が含まれていたため、WCAG 検証 が失敗していました。

<!-- MC-40528-->

* アプリケーション でカタログイベントの日付が UTC タイムゾーンで保存されるようになりました。 以前は、アプリケーションによって保存されたカタログの日付イベントユーザーのタイムゾーンが表示されていました。

<!-- MC-39765-->

* ゲスト買い物客が 無効 アドレスを使用するときに、アプリケーションでシステムログが生成されるエラーをスローしなくなりました。 以前は、ページ: `No such entity with addressId` がレンダリングされずに、アプリケーションが断続的にこのエラーを表示していました。 [GitHub-15115](https://github.com/magento/magento2/issues/15115)

<!-- MC-39747-->

* サポートされていない形式画像の画像サイズ変更プロセスが停止しなくなりました。 以前は、サポートされていない画像形式 `catalog:images:resize` が発生すると、プロセスは停止し、アプリケーションに次のエラーが表示されました。 `bin/magento catalog:images:resize Unsupported image format`。

<!-- ENGCOM-8925-->

* ロゴ画像の WEBP および AVIF サポートが管理に追加されました。 [GitHub-32495](https://github.com/magento/magento2/issues/32495)

<!-- MC-38156-->

* `media_gallery`属性の範囲を変更することはできなくなりました。以前は、 `media_gallery` 属性の範囲を `global` に戻すと、アプリケーションでエラーがスローされていました。

<!-- MC-36818  magento/partners-magento2ee#470-->

* スケジュールされた更新として追加されたその他の関連ヘルプ、更新の終了日以降、ストアフロント製品ページに表示されなくなります。 [GitHub-469](https://github.com/magento/partners-magento2ee/issues/469)
...
* データベースストレージ方法を使用してカテゴリを保存すると、カテゴリー画像が `catalog/tmp/category` ディレクトリから `catalog/category` ディレクトリに期待どおりにコピーされるようになりました。 これで、 `media_storage_file_storage` テーブルの画像行も正しい `directory_id`になります。 [GitHub-11995](https://github.com/magento/magento2/issues/11995)

<!-- MC-24768-->

* マーチャントは、CMS階層に変更を送信した後、期待どおりに無効化されたキャッシュについて通知されるようになりました。 以前は、CMS アプリケーション階層の保存中にキャッシュが無効になりませんでした。

<!-- MC-24725-->

* 買い物かごの買い物かごセクションへの冗長なAJAX要求が減少しました。 以前は、アプリケーションで買い物かごの小計が適切に読み込まれず、買い物かごリロードが再度トリガーされました。
...
* 保留支払手数料は、ギフトカードがキャンセルされたときに予想どおりにギフトカードから削除されるようになりました。 以前は、ギフトカードが削除された後に `cron` を実行すると、アプリケーションでエラーが発生していました。

<!-- MC-37658-->

* 買い物客は、複数の住所でチェックアウトするときに、注文の確認ページ注文からギフトカードを削除できるようになりました。 以前は、買い物客がカードを選択してから **削除** をクリックしたときに、アプリケーションギフトカードが注文から削除されませんでした。

<!-- MC-34407-->

* アプリケーション買い物かごを追加した後にギフトカードの値を変更すると、買い物かご内のギフトカードの新しい価格が表示されるようになりました。

<!-- MC-32651-->

* 価格帯 検証 ロジックがギフトカード作成ページに追加されました。 以前は、管理者は、最大値を超える最小値を持つカードを作成できました。 [GitHub-493](https://github.com/magento/partners-magento2ee/issues/493)
...
* `setGiftOptionsOnCart` の突然変異でギフトメッセージが正しく作成されるようになりました。[GitHub-388](https://github.com/magento/partners-magento2ee/issues/388)
...
* ストアビューのギフト包装ラベルの翻訳が期待どおりに機能するようになりました。
...
* `addConfigurableProductsToCart` ミューテーションは、指定した製品の正しいサムネールを返すようになりました。 以前は、親製品のサムネイルが返されていました。

<!-- ENGCOM-8477-->

* 特別価格期間が将来の日付に設定されている場合、 `products` クエリは製品の特別価格を公開しなくなりました。 [GitHub-30210](https://github.com/magento/magento2/issues/30210)、 [GitHub-29631](https://github.com/magento/magento2/issues/29631)

<!-- magento-engcom/magento2ee#576-->

* `customer` クエリ 応答に、`OrderTotal` オブジェクトの一部として `total_giftcard` 属性が含まれるようになりました。

<!-- MC-41084 ENGCOM-9042 -->

* `products` クエリで、階層価格が設定されていないアイテムの価格帯に関する誤った情報が返される原因となっていた問題を修正しました。[GitHub-32279](https://github.com/magento/magento2/issues/32279)

### 画像

<!-- MC-38951-->

* POST `/V1/product/:sku` エンドポイントを使用してバンドル製品を作成または更新する際に、required_options と has_options バンドル属性を必要に応じて設定できるようになりました。 以前は、これらのカスタム属性は 1 に設定しようとしても、0 (ゼロ) に設定されていました。

### インポート/エクスポート

<!-- MC-41576-->

* 書き出し ページ エンティティ 属性 グリッド内の製品エンティティの カテゴリー ID フィルターが期待どおりに機能するようになりました。

<!-- MC-41439 ENGCOM-9005-->

* すべての子商品が在庫切れの場合、グループ化された商品の在庫ステータスが期待どおりに更新され、在庫切れになるようになりました。 [GitHub-32647](https://github.com/magento/magento2/issues/32647)

<!-- MC-41184-->

* 読み込みによって子製品のストックステータスが更新される際に、設定可能な製品のストックステータスが期待どおりに自動的に更新されるようになりました。 以前は、子製品の在庫ステータスがインポートによって更新されたときに、製品の在庫ステータスが自動的に更新されませんでした。
...
* 製品が `None` または `0` の値でインポートされたかどうかを `None` す `tax_class_name` 製品税区分が設定されるようになりました。 以前は、CSV ファイルに `product tax_class_name` が `None` まれている場合、アプリケーションによって新しい税金区分 `None` が作成され、既存の税金区分が複製されていました。 CSVファイルに `product tax_class_name` が `0` されている場合、アプリケーションはその値を無視し、輸入後に製品の税クラスは変更されませんでした。
...
* インデクサーモードが **保存時に更新** に設定されている場合、REST PUT `/V1/products/:sku/links` リクエストを使用して商品をリンクした際に、ストアフロント検索結果で商品が期待どおりに使用できるようになりました。
...
* 特例文字インデクサーは、サブスクリプションに異なるエンティティ列名を使用できるようになりました。 以前は、データベース トリガーでは、指定されたインデクサーではなく、最後に設定されたインデクサーの列名を **スケジュールで更新** に使用していました。 [GitHub-21853](https://github.com/magento/magento2/issues/21853)

<!-- ENGCOM-8887-->

* アプリケーションは、リクエストの影響を受けないリロード顧客データセクション(`Magento_Customer/js/section-config`)にAJAX要求を送信しなくなりました。 [GitHub-31948](https://github.com/magento/magento2/issues/31948)

<!-- MC-39569-->

* カテゴリ内の製品を追加、削除、または並べ替えた後、 `catalog_category_product` と `catalogsearch_fulltext` (およびそれらの依存関係) のインデクサーが無効になりなくなりました。 以前は、これらの不注意による削除によって、サイトの完全なインデックス再作成がトリガーされていました。 フラットカタログが有効になっていない場合、この条件下では完全なインデックスの再作成ができないようになりました。
...
* `web-token/jwt-framework` パッケージで欠落していた依存関係を`magento/module-jwt-framework-adapter`に追加しました。[GitHub-32578](https://github.com/magento/magento2/issues/32578)
...
* アプリケーションは、実稼働モードで `bin/magento` コマンドを実行しているときにエラーが発生した場合に、より有益なエラーを表示するようになりました。 以前は、アプリケーションにエラーメッセージが表示されなかったり、情報が不足しているメッセージが表示されたりしていました。 [GitHub-32786](https://github.com/magento/magento2/issues/32786)
...
* アプリケーションでは、各cookie個別のコンテキストとしてログに記録されなくなりました。 `$_COOKIE`配列も文字列に変換されています。以前は、各 cookie が個別のコンテキストとしてログに記録されていたので、cookie の数が 50 を超えると、アプリケーションは次のメッセージを記録していました。`Unable to send the cookie. Maximum number of cookies would be exceeded` [GitHub-31334](https://github.com/magento/magento2/issues/31334)
...
* アプリケーションは、メールが送信された領域に関係なく、`frontend` で支払いブロックをレンダリングするようになりました。 （支払いブロックがレンダリングされる前に、現在の領域が `frontend` としてエミュレートされるようになりました。） 以前は、支払いブロックはメールが送信されたエリアでレンダリングされていました。 その結果、営業メールが管理者からトリガーされたか、REST API によってトリガーされたかにかかわらず、アセット の URL が間違った領域 (`webapi_rest` または `adminhtml`) から読み込もうとしました。

<!-- MC-40810-->

* アプリケーションは、ダウンロード可能な製品のリンクを、チェックアウト時に指定された電子メール アドレスに送信するようになりました。 以前は、ゲスト買い物客 Express Checkoutを使用し、注文を送信してチェックアウトするために異なる電子メールアドレスを入力するPayPal、アプリケーションダウンロード可能な製品リンク最初のアドレスに送信されていました。

<!-- MC-38051-->

* アプリケーションには、複数の通貨をサポートする展開で支払メソッドページで使用可能なストアクレジットの正確な値が表示されるようになりました。

<!-- MC-37820-->

* お支払い方法リストは、ゲストの買い物客がチェックアウト中に注文の配送先住所を別の国に変更した場合に、期待どおりに更新されるようになりました。 以前は、住所を変更しても請求可能な支払い方法の更新はトリガーされませんでした。
...
* `/V1/guest-carts/examplecartid/items` 呼び出しは、要求されたストア表示を返します。以前は、リクエストされた表示ではなく、ストア内の最初のストア返されていました。
...
* 管理者 **レポート** > **レビュー** > **製品別** の最終レビュー日に正しいレビュー日が表示されるようになりました。 以前は、アプリケーションにレビュー日ではなく製品の作成日が表示されていました。

### 返品承認 (RMA)

<!-- MC-41866-->

* RMA `resolution` 属性は、属性フィールド値 `required` が **no** に設定されている場合、必須として表示されなくなりました。

<!-- MC-41808-->

* 管理者から返品する複数の注文を選択できるようになりました。 以前は、複数の返品を選択し、「アクション」ドロップダウンメニューから「クローズ」を選択した後に「**送信**」をクリックすると、アプリケーションがエラーをスローしていました。
...
* レビューは、管理者がストアとは異なるドメインからレビューを承認して保存した後、正しいストア ID で保存されるようになりました。 [GitHub-17510](https://github.com/magento/magento2/issues/17510)
...
* Admin 新規作成 Order ページ がリロードされた後、チェックアウト ワークフローの Payment &amp; Shipping 情報 セクションで支払い方法ラジオボタンが消えなくなりました。 [GitHub-32106](https://github.com/magento/magento2/issues/32106)
...
* アプリケーション、新しい注文の電子メールを顧客に送信するときにランダムなデータベースのデッドロックが発生しなくなりました。 以前は、更新された SQL 列ではなく、アプリケーションオブジェクト全体とその関連オブジェクトを保存するためにデッドロックが発生していました。 アプリケーションに次のエラーが表示されました：`SQLSTATE[40001]: Serialization failure: 1213 Deadlock found when trying to get lock; try restarting transaction`。 [GitHub-31090](https://github.com/magento/magento2/issues/31090)
...
* 「請求書のコピーを E メールで送信 **ボタンがオフの場合、アプリケーションはオーダー請求書のコピーを E メールで送信** なくなります。 以前は、アプリケーションは **顧客** に電子メールを送信し **送信請求書の電子メールコピー**。 [GitHub-28511](https://github.com/magento/magento2/issues/28511)
...
* 配送ラベルは、注文が基本通貨以外の通貨で行われた場合に、複数の通貨をサポートするストアの注文通貨ではなく、期待どおりに基本通貨を使用するようになりました。 [GitHub-31891](https://github.com/magento/magento2/issues/31891)

<!-- MC-40678 magento/magento2#31925-->

* 複数配送ページから移動した後、チェックアウト中に買い物客が買い物かごページに戻ったときに、アプリケーションに正しい注文小計が表示されるようになりました。 [GitHub-31889](https://github.com/magento/magento2/issues/31889)

<!-- MC-40541 magento/magento2#31841 -->

* 管理注文の作成時に請求情報を編集しても、デフォルトの配送先住所と請求住所が異なる顧客の配送情報は変更されなくなりました。 [GitHub-31786](https://github.com/magento/magento2/issues/31786)

<!-- MC-37385 ENGCOM-8578-->

* アプリケーション、買い物客が&#x200B;**配送方法と料金を取得**&#x200B;をクリックし、管理者から注文を作成するときに配送方法を選択したときに、&lbrack;**コメントを追加**&#x200B;チェックボックスをオフにしなくなりました。[GitHub-30256](https://github.com/magento/magento2/issues/30256)

<!-- MC-36425-->

* 買い物客は、戻るブラウザーボタンを使用して、複数の住所を持つ注文をチェックアウトしながら、配送を選択メソッドページに戻ることができるようになりました。 以前は、アプリケーションに破損した「出荷方法の選択」ページが表示されていました。 [GitHub-30268](https://github.com/magento/magento2/issues/30268)
...
* 定額配送方法の料金は、チェックアウト中に買い物かご価格ルールが適用された場合にゼロにならないようになりました。 [GitHub-21832](https://github.com/magento/magento2/issues/21832)
...
* アプリケーションは、カテゴリのスケジュール更新のプレビュー中に、すべての製品 ID を含む大規模なデータベース クエリを実行しなくなりました。 代わりに、カテゴリフィルターが製品コレクションに適用され、カテゴリのステージングプレビュー用の一時テーブルが作成されます。 以前は、アプリケーションによってすべての製品がステージングプレビュー用の一時テーブルに挿入されたため、パフォーマンスが低下していました。
...
* 管理者が既存顧客の注文を作成するときに、アプリケーションは、新しい注文の管理ページのアドレス情報セクションの請求と配送先住所の両方の **VAT 番号** 入力フィールドに保存されたVAT番号を事前入力するようになりました。 [GitHub-31846](https://github.com/magento/magento2/issues/31846)

<!-- ENGCOM-8617 magento-engcom/magento2ee#558-->

* `customer/create_account/vat_frontend_visibility`アプリケーションを有効にすると、顧客 登録 ページに **VAT 番号** フィールドが表示されるようになりました。[GitHub-31326](https://github.com/magento/magento2/issues/31326)
...
* `Catalog`、`CatalogUrlRewrite`、`LoginAsCustomer` の各モジュールの実行時間を短縮するために、テストから `cache:flush` コマンドを削除しました。 [GitHub-31056](https://github.com/magento/magento2/issues/31056)
...
* ソースのページネーションが、管理者のシップメント作成中に期待どおりに表示されるようになりました。

<!-- MC-40240-->

* カスタム 日付属性を使用して製品をフィルター処理し、Admin ユーザー ロケールが `en_GB`場合、製品グリッド フィルターが正しく機能するようになりました。

<!-- MC-39279-->

* ピンチしてズームするためのジェスチャが、iOS デバイス の製品ページ画像ギャラリー拡大鏡で期待どおりに機能するようになりました。

<!-- MC-38787-->

* 管理商品グリッドの検索結果のページネーションが、検索ごとに 1 つずつページ開始されるようになりました。

<!-- MC-38423-->

* ズーム時の買い物かご表示の異常を解消しました。 以前は、このページズームすると表示要素が重なり合っていました。

<!-- MC-38235 magento/partners-magento2ee#492 -->

* 管理者招待ページ (管理者 **マーケティング** > **プライベート セールス** > **招待**) のタブに関する整列問題が解決されました。 以前は、顧客によって承諾された招待状のTab表示の位置がずれていました。 [GitHub-333](https://github.com/magento/partners-magento2ee/issues/333)

<!-- MC-38226 magento/magento2#32008-->

* PayPal Express Checkout を使用したチェックアウト中に表示される注文レビューページが正常に読み込まれるようになりました。 以前は、テンプレートには未使用の **Update 配信 メソッド** ボタンが含まれており、ページレンダリング中にスクリプトによって部分的にしか非表示されていませんでした。 [GitHub-32006](https://github.com/magento/magento2/issues/32006)

<!-- MC-37941 magento/magento2#31892-->

* 製品ページのカスタマーレビューセクションの評価星とレビューテキストは、長いレーティング名が存在する場合に適切な間隔が空けるようになりました。 [GitHub-31890](https://github.com/magento/magento2/issues/31890)

<!-- MC-37688-->

* オプションは、[請求書]、[出荷]、および [クレジット メモ] ページの アクションアーカイブ ドロップダウン リストに期待どおりに表示されるようになりました。

<!-- MC-23729  magento/magento2#31549-->

* `datetime`コンポーネント`timeOnly`オプションが&#x200B;**はい**&#x200B;に設定されている場合、アプリケーションに正しい時間値が表示されるようになりました。[GitHub-23157](https://github.com/magento/magento2/issues/23157)
...
* **削除レイアウト更新**&#x200B;ボタンは、管理者&#x200B;**コンテンツ** > **ウィジェット**&#x200B;から追加したすべてのレイアウトで期待どおりに機能するようになりました。以前は、このボタンは、複数のレイアウトを追加するときに最初に選択したレイアウト以外のレイアウトでは機能しませんでした。 [GitHub-30286](https://github.com/magento/magento2/issues/30286)
...
* マーチャントがパスワードの表示と非表示を切り替えることができるチェックボックスが次のページに追加されました。
   ...
   * カスタマー設定 新規 パスワード [GitHub-31557](https://github.com/magento/magento2/issues/31557)

<!-- ENGCOM-8545 -->

* 属性セットを変更した後に管理者がグループ化された製品セットに製品を追加したときに表示されるグリッド セルの冗長ヘッダーを削除しました。 [GitHub-30911](https://github.com/magento/magento2/issues/30911)
...
* 親カテゴリでURLキーの **デフォルト値を使用** チェックボックスが有効になっている場合、アプリケーションは子カテゴリのURLパスを正しく生成するようになりました。 以前は、複数ストア表示デプロイメントで異なる URL キーを使用すると、階層内のカテゴリを移動すると不正な `url_path` 値になっていました。 [GitHub-16202](https://github.com/magento/magento2/issues/16202)

<!-- MC-40371-->

* カテゴリ階層でカテゴリを移動すると、アプリケーションですべてのストア範囲に割り当てられているカテゴリが更新される `url_path` ようになりました。 以前は、階層内のカテゴリを移動すると間違った `url_path`が発生していました。

<!-- MC-39463-->

* カスタム URL を使用して管理者から作成された URL リダイレクトが正常に機能するようになりました。 以前は、GraphQL は GraphQL `urlResolver` クエリからのレスポンスをキャッシュし、URL書き換えの更新後に古い値を返していました。

<!-- MC-38931  magento/magento2#31106-->

* Web サイトから製品が削除されると、製品URLの書き換えが期待どおりに削除されるようになりました。 [GitHub-24184](https://github.com/magento/magento2/issues/24184)

<!-- ENGCOM-8719 -->

* `app/code/Magento/UrlRewrite/Model/ResourceModel/UrlRewriteCollection.php` コレクション の `addStoreFilter` 機能に、`store_id`への`main_table`参照を追加しました。以前は、`store_id`列も含むテーブルのコレクションに`join`が追加されるたびに問題が発生していました。[GitHub-31853](https://github.com/magento/magento2/issues/31853)

<!-- ENGCOM-8894 -->

* `PUT /V1/products/:sku` REST エンドポイントで製品が再生成されるようになりURL期待どおりに書き換えられるようになりました。以前は、エンドポイントで商品 `url_key` 値が再生成されていましたが、書き換えURLは再生されませんでした。 [GitHub-30316](https://github.com/magento/magento2/issues/30316)
...
* 高度な JavaScript バンドルを有効にして使用する場合、ビデオは期待どおりに製品ギャラリーで利用できるようになりました。 [GitHub-32501](https://github.com/magento/magento2/issues/32501)

<!-- ENGCOM-8532-->

* 設定可能な商品に属するシンプルな商品のYouTube ビデオが、正常に機能するようになりました。 以前は、画像は位置ではなく ID で並べ替えられていました。 その結果、一部のビデオは画像のように扱われていました。 [GitHub-29690](https://github.com/magento/magento2/issues/29690)

### ビジュアルマーチャンダイザー

<!-- MC-40797-->

* ビジュアルマーチャンダイザーを使用した管理カテゴリページの読み込みのパフォーマンスが向上しました。 Admin カテゴリ ページを読み込むときに、大きなデータベースクエリが生成されなくなりました。

### Web API フレームワーク

<!-- MC-41758-->

* カタログ API は、新しい値を追加し、古い値を削除することで、製品のカスタム オプション値を正しく更新するようになりました。 以前は、API で古い値は削除されませんでした。

<!-- MC-34431-->

* POST `/V1/guest-carts/:cartId/billing-address` で、アドレス ID が文字列ではなく整数として返されるようになりました。
...
* TinyMCEのCMS階層ノードリンクウィジェットは、 **アンカーカスタムテキスト** フィールドが空の場合に期待どおりに階層リンクをレンダリングするようになりました。
...
[Composer](../../../installation/composer.md) を使用して Commerce 2.4.3 Adobe Systemsインストールできます。

## 移行ツールキット

データ 移行ツールは、既存の Magento 1.x ストア データを Magento 2.x に転送するのに役立ちます。このコマンド ライン インターフェイスには、検証、進行状況トラッキング、ログ、およびテスト機能が含まれています。 インストール手順については、「 [データ 移行ツールのインストール](../../../tools/data-migration-tool/install.md)を参照してください。 [データ移行リポジトリー](https://github.com/magento/data-migration-tool)の調査や貢献を検討してください。

[Code Migration Toolkit](https://github.com/magento-commerce/code-migration) は、既存の Magento 1.x ストア拡張機能とカスタマイズを Magento 2.x に移行するのに役立ちます。コマンドラインインターフェイスには、Magento 1.x モジュールとレイアウトを変換するためのスクリプトが含まれています。

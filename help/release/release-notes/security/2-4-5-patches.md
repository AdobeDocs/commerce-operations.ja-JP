...
>[!IMPORTANT]
>
>MySQL 8.0 は、2026 年 4 月 30 日（PT）よりサポート終了（EOS）となります。
>
>この日以降、Adobe Commerce 2.4.5 では互換性がなくなる
>mysql 8.0 以降にリリースされた MySQL バージョンのサポート。Adobeが実行しない
>このAdobeで新しい MySQL メジャーバージョンの検証またはサポートの提供
>Commerceのリリースライン。
>
>バージョン 2.4.5 を実行しているAdobe Commerceのオンプレミス環境のお客様は、すべて非常に強い傾向があります
>データベース サーバーを互換性のある MariaDB バージョンに移行することをお勧めします。

## 2.4.5-p16

Adobe Commerce 2.4.5-p16 セキュリティリリースは、2.4.5 の以前のリリースで特定された脆弱性に対するセキュリティバグ修正を提供します。

セキュリティのバグ修正の最新情報については、[Adobe セキュリティ速報 APSB26-05](https://helpx.adobe.com/security/products/magento/apsb26-05.html) を参照してください。
...
#### Apache ActiveMQ Artemis との互換性

RabbitMQ 4 に関連するサポート終了のリスクがあるため、Adobe Commerceでは、メッセージキューインフラストラクチャの長期的な安定性とサポートを確保するために、Apache ActiveMQ Artemis への戦略的移行を開始しました。

* Adobe Commerce 2.4.5 で、最新バージョンの Apache ActiveMQ Artemis がサポートされるようになりました。
* RabbitMQ 4.1 との互換性は、既存の MQ サービスを引き続き利用したいお客様をサポートするために維持されています。
* クラウドネイティブデプロイメント用のAWS ActiveMQ を含め、ActiveMQ が Adobe Commerce Cloud で完全にサポートされるようになりました。

#### 2.4.5-p16 と MariaDB 10.11 の互換性

Adobe Commerce 2.4.5-p16 は、MariaDB 10.6 を引き続き完全にサポートしながら、MariaDB 10.11 との互換性が検証されています。現在 2.4.5-px を実行しているマーチャントは、2.4.5-p16 にアップグレードして、MariaDB 10.6 から MariaDB 10.11 に安全に移行できます。
...
>[!NOTE]
>
>2.4.5 用の拡張サポートセキュリティパッチは、Adobe Commerceのお客様のみが利用できます。 これらのパッチは、Magento Open Source コードベースでは使用できません。 [ 拡張サポート ](https://experienceleague.adobe.com/en/docs/commerce-operations/release/planning/lifecycle-policy#extended-support) を参照してください。
...
セキュリティのバグ修正の最新情報については、[Adobe セキュリティ速報 ](https://helpx.adobe.com/security/products/magento/apsb23-35.html) を参照してください。

...
修正された問題の最新の説明については、[Adobe セキュリティ速報 ](https://helpx.adobe.com/security/products/magento/apsb22-38.html) を参照してください。

#### その他のセキュリティ強化

このリリースのセキュリティ強化により、次のような最新のセキュリティのベスト プラクティスへの準拠が向上します。

* reCAPTCHA のサポートが、ウィッシュリスト共有、新規作成カスタマーアカウント、ギフトカードのフォームに追加されました。 <!-- AC-1144-->
...
* TinyMCE (5.10.2)。 以前のバージョンのTinyMCE(v5.9.2以前)では、特別に細工されたURLまたは特別に細工されたURLを含む画像が更新されると、任意のJavaScriptが実行される可能性がありました。 <!-- AC-2039 1784 1789-->
...
* 製品の在庫切れになっても、`cart` クエリが null 応答を返さなくなりました。 エラーメッセージを含む新しい `errors` 要素が応答に導入されました。 以前は、在庫切れの商品を含むクエリを実行すると、「Sourceを開く」アプリケーションの応答の「`items`」セクションに `null` 値が表示されていました。 [ 買い物かごクエリ ](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/queries/cart/) を参照してください。
...
* Magento Open Sourceでは、管理者に `preg_replace()` エラーが表示されなくなりました。 3 番目の引数（`$subject`）の型は、`bool` ではなく `array|string` になりました。
...
* **検索結果で使用** レイヤナビゲーション設定が無効になっている場合、`products` クエリは属性を集計として返さなくなりました。 [GitHub-33318](https://github.com/magento/magento2/issues/33318)
...
* `ProductInterface` の `new_from_data` と `new_to_datefields` は、非推奨（廃止予定）ではなくなりました。 [GitHub-34783](https://github.com/magento/magento2/issues/34783)
...
* `gift_message` オブジェクトが応答で指定されているがギフトメッセージが存在しない場合、`customerOrders` クエリは期待どおりに応答するようになりました。 以前は、クエリは次のメッセージを返していました：`Can't load gift message for order is returned`。 [GitHub-28957](https://github.com/magento/magento2/issues/28957)
...
* Magento Open Source では、名前の先頭に数字が付加されているストアビューから作成された注文のクレジットメモグリッドページにクレジットメモが表示されるようになりました。 以前は、アプリケーションを開く出力元にエラーが表示されました: `The store that was requested wasn't found. Verify the store and try again. Exception in /var/www/html/vendor/magento/module-store/Model/StoreRepository.php:75`。 [GitHub-35122](https://github.com/magento/magento2/issues/35122)
...
* 既定の範囲に値がない (ただし、ストア表示 レベルで定義されている) 価格属性は、適切にインデックス付けされるようになりました。 以前は、EAV テーブルから価格属性値を取得する SQL 式では、値が既定の範囲で定義されていないシナリオアカウントされていませんでした。

<!-- ACP2E-313-->

* 商品詳細ページに記載されている価格は、選択した量によって異なる階層価格のチェックアウトワークフローに記載されている価格と同じになりました(たとえば、2 つのアイテムと 5 つのアイテムの購入に基づいて異なる価格の商品)。 以前は、チェックアウト価格には最低製品数量の価格が反映されていました。

### ReCAPTCHA

* ログインページとアカウントページの作成 **送信** ボタンは、ReCaptchaが完全に読み込まれるまで非アクティブになります。

### 役割

* ロールリソースタブで リソース Access が **特例文字** に設定されている場合、**Store** > **Configuration** > **サービス**&#x200B;ページ に期待どおりに Web API Magento Open Source情報が表示されるようになりました。[GitHub-35506](https://github.com/magento/magento2/issues/35506)
...
* 検索可能なバックエンド `type` 属性として設定されている場合、Elasticsearchのクエリ `int` 期待どおりに動作するようになりました。 以前は、applicationOpen Sourceが `Elasticsearch\Common\Exceptions\BadRequest400Exception` 例外をスローしていました。
...
* 買い物かご内の製品に対して固定税(FPT)が正しく表示されるようになりました。 以前は、買い物かご内の複数の製品に&#x200B;**固定製品税 (FPT) と**&#x200B;**適用税 To FPT**&#x200B;が有効になっている場合、すべての FPT が買い物かご内の最後の製品に割り当てられ、他の製品ではリセットされていました。

<!-- ACP2E-350-->

* チェックアウト ワークフローの注文内容セクションの固定物品税 (FPT) の合計が正しく計算されるようになりました。
...
* `cart`クエリ`subtotal_with_discount_excluding_tax`返却時に税金が含まれなくなりました。[GitHub-33905](https://github.com/magento/magento2/issues/33905)
...
* `Magento_LoginAsCustomerAdminUi`が有効になっていて、**ストア表示ログイン先**&#x200B;が手動選択に設定されている場合、管理者から顧客を作成できるようになりました。以前は、applicationOpen 出力元で次のエラーがスローされていました: `(Magento\Framework\Exception\LocalizedException): Unable to get Customer ID`。 [GitHub-33096](https://github.com/magento/magento2/issues/33096)

<!-- AC-2076-->

* 買い物客が製品画像ギャラリーの最後のサムネール画像に到達すると、次へ矢印が無効になりました。
...
* 管理者は、現在のユーザーの有効期限日付を 2038 よりも高く設定し、ユーザー正常に保存できるようになりました。 以前は、有効期限日付が変更されたユーザーは、ログ後に再度ログインできませんでした。

<!-- AC-751-->

* Magento Open Source管理者が管理者 **店舗** > **属性**  > **顧客アドレス**&#x200B;で過剰な番地を含む住所を保存しようとすると、情報エラーメッセージが表示されるようになりました。  これで、管理者は余分な住所情報を削除し、住所を正常に保存できます。 以前は、applicationOpen 出力元余分な行をコミットしましたが、データは保存されませんでした。

<!-- ACP2E-546-->

* データベースおよびローカルストレージの製品リスト表示設定が更新されました。 特例文字グリッドビューは、ページリロードおよび表示の変更時に保存されるようになりました。
...
* CSP が `restrict mode` に設定されている場合、`banner` 要素のバックグラウンドで実行されるように Vimeo ビデオを設定できるようになりました。 以前は、applicationOpen SourceがJavaScript エラーをスローしていました。
...
* `POST /V1/salesRules/`エンドポイントを介して作成されたカート価格ルールは、ステータスが無効から有効に変更された後も、既存のクーポンコード値を保持するようになりました。[GitHub-35298](https://github.com/magento/magento2/issues/35298)
...
* `POST /V1/products/special-price`エンドポイントでの新しい特別価格表の作成が、期待どおりに機能するようになりました。以前は、エンドポイントから `Future Update already exists in this time range. Set a different range and try again` 件のエラーが返されていました。

<!-- AC-1169-->

* `/V1/products/base-prices`エンドポイントが&#x200B;**カタログ価格モード - ウェブサイト**&#x200B;で期待どおりに機能するようになりました。[GitHub-30132](https://github.com/magento/magento2/issues/30132)

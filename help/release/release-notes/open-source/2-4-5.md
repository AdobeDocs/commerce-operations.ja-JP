...
* 製品の在庫切れになっても、`cart` クエリが null 応答を返さなくなりました。 エラーメッセージを含む新しい `errors` 要素が応答に導入されました。 以前は、在庫切れの商品を含むクエリを実行すると、「Sourceを開く」アプリケーションの応答の「`items`」セクションに `null` 値が表示されていました。 [ 買い物かごクエリ ](https://developer.adobe.com/commerce/webapi/graphql/queries/cart.html) を参照してください。

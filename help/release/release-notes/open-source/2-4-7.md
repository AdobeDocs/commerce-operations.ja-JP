...
* **PHP 8.3 互換**。 このリリースでは、PHP 8.3 のサポートが導入されています。Magento Open Sourceでは、PHP 8.3 と 8.2 の両方をサポートするようになりました。PHP 8.2 は、2025 年 12 月のサービス終了（EOS）日までサポートされます。 2025 年 12 月以降、2.4.7 デプロイメントを実行しているすべてのマーチャントは、PHP 8.3 に移行する必要があります。<!-- AC-8988 -->
...
* **RabbitMQ 3.13 のサポート**。 このリリースは最新バージョンの RabbitMQ 3.13 と互換性があります。互換性は RabbitMQ 3.11 および 3.12 に残っており、それぞれ 2024 年 8 月と 2024 年 12 月までサポートされていますが、Adobeでは、RabbitMQ 3.13 でのみMagento Open Source 2.4.7 を使用することをお勧めします。<!-- AC-9659 -->

* **Composer 2.7.x**。 Composer 2.2.x との互換性は維持されます。<!-- AC-9663 -->

* **Varnish cache 7.4 のサポート**。 このリリースは、最新バージョンの Varnish Cache 7.4 と互換性があります。6.0.x および 7.2.x バージョンとの互換性は維持されますが、Magento Open Source 2.4.7 は Varnish Cache version 7.4 または version 6.0 LTS でのみ使用することをお勧めします。<!-- AC-9662 -->
...
Magento Open Source コアコード内のすべてのJavaScript ライブラリと NPM の依存関係が、利用可能な最新バージョンに更新されました。 すべての Laminas ライブラリの依存関係は、PHP 8.3 と互換性のある最新バージョンに更新されました。<!-- AC-9667 9665 -->

#### 追加のアップグレード

* Commerce UPS XML API ゲートウェイは、UPS が自社の {API セキュリティ モデル ](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/delivery/shipping-carriers/ups) に対して行っている更新をサポートするために、新しい [0}Commerce UPS REST API](https://developer.ups.com/oauth-developer-guide) に移行されました。 [（UPS は、すべての API に対して OAuth 2.0 セキュリティモデル（ベアラートークン）を実装しています。） 以前のCommerce UPS XML API はすべて、Magento Open Source 2.4.7 コードベースから削除されました。<!-- AC-8918 -->

* FedEx のMagento Open Source [integration](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/delivery/shipping-carriers/fedex) は、従来の FedEx WSDL Web サービスから最新の FedEx RESTful API に移行されました。 FedEx Web Services Tracking、Address Validation、および Validate Postal Codes WSDLS は、2024 年 5 月に廃止されます。<!-- AC-1470 -->
...
* Temando 出荷モジュールは、コア Magento Open Source コードベースから削除されました。 この機能はMagento Open Source 2.4.4 で廃止されました。<!-- AC-8130 -->
...
* [ブレーントリー](https://experienceleague.adobe.com/docs/commerce-admin/stores-sales/payments/braintree.html)
...
* `GET /V1/products/attributes/{attributeCode}/is-filterable`。 パスパラメータ: `attributeCode` (`String`)。<!-- ACP2E-2275 -->
...
* 不要なキャッシュ操作がセットアッププロセスから削除されました。 以前は、Magento Open Source は `bin/magento setup:db-data:upgrade`または`bin/magento setup:upgrade` の実行時に不必要に構成をディスクに書き込み、セットアップ中に som モジュールに問題を発生させていました。 [GitHub-38124](https://github.com/magento/magento2/issues/38124)
...
* `bin/magento setup:install``app/etc/config.php`を削除した後、 は正常に完了するようになりました。以前は、インストール中に見つからないファイルが再生成されず、Magento Open Sourceでエラーがスローされていました。 [GitHub-37805](https://github.com/magento/magento2/issues/37805)
...
* 配送方法を条件として設定した買い物かご価格ルールが適用される定額配送方法を選択した後、買い物客がクーポンを適用すると、クーポンが注文に正しく適用されるようになりました。 以前は、買い物かごページに「クーポンが無効です」というエラーが表示され、クーポンが適用されませんでした。 [GitHub-34866](https://github.com/magento/magento2/issues/34866)

<!-- AC-8596 -->

* 買い物客がオートコンプリートを使用して配送先住所を入力しクロム配送方法ブロックがリロードされる前に&#x200B;**次へ**&#x200B;をクリックした場合、**[!UICONTROL My billing and shipping address are the same]**&#x200B;チェックボックスはデフォルトでオンのままになります。[GitHub-33725](https://github.com/magento/magento2/issues/33725)
...
* ローカル ストレージ 内のミニ買い物かごおよび顧客 データは、セッションがサーバーから削除されたときにリセットされるようになりました。 以前は、セッションファイルが削除されたときにこのデータが表示されていました。
...
* 買い物かご税および送料の見積もりに、デフォルトの配送先設定が正確に反映されるようになりました。 以前は、デフォルトの税金宛先計算設定を設定し、州/地域と郵便番号を指定した場合、**[!UICONTROL Estimate Tax and Shipping]** の下の買い物かごに ZIP 値のみが表示されていました。 これはMagento Open Source管理者からデフォルトの地域 IDを設定する条件ステートメントを呼び出す前に、地域 IDを未定義に設定したことが原因です。

<!-- ACP2E-1547 -->

* 永続的な買い物かごが有効な場合、チェックアウト中に Magento Open Source でこのエラーがスローされなくなりました。 `Invalid state change requested`。 以前は、このエラーがスローされると、同じ `quote_id` を持つ複数の注文が作成されることがありました。
...
* ある配送方法に関連する買い物かご価格ルールが店舗に対して設定され、チェックアウト中にこの配送方法が別の配送方法に変更された場合、買い物かご価格ルールが期待どおりに適用されるようになりました。 以前は、`sales_order_item` テーブルで `applied_rule_ids` 値が変更されることはなく、買い物かごの価格ルールが注文に適用されていませんでした。

<!-- AC-2044 -->

* 配送方法を条件として設定した買い物かご価格ルールが適用される定額配送方法を選択した後、買い物客がクーポンを適用すると、クーポンが注文に正しく適用されるようになりました。 以前は、買い物かごページに `Coupon is Not valid` エラーが表示され、クーポンが適用されていませんでした。 [GitHub-34866](https://github.com/magento/magento2/issues/34866)
...
* Magento Open Source製品の詳細ページで製品の色が変更されたときに重複画像が表示されなくなりました。  [GitHub-36243](https://github.com/magento/magento2/issues/36243)

<!-- ACP2E-2504 -->

* 大規模なマルチストア環境で新しいストアを追加した後、または既存のを変更した後に、Web サイト固有の値を同期するためのアルゴリズムを最適化しました。

<!-- ACP2E-2631 -->

* キャッシュされたイメージ ファイルのクリーンアップ プロセスが改善され、 `pub/media/catalog/product` フォルダーに 非表示 ディレクトリが作成されなくなりました。 以前は、非表示ディレクトリが原因で、クリーンアップ操作中にディスク容量の問題が発生していました。 操作を終了すると、削除されなかった一時的な非表示ディレクトリが残る可能性があります。

<!-- AC-10316 -->

* バンドル製品の価格が正しく計算されるようになり、価格計算時の操作の順序の問題が解決されました。 [GitHub-35665](https://github.com/magento/magento2/issues/35665)

<!-- AC-6996 -->

* `catalog_product_price`インデックス クエリから不要な句が削除され、カタログが大きいストアでのこのインデクサーのパフォーマンスが向上しました。[GitHub-32382](https://github.com/magento/magento2/issues/32382)

<!-- ACP2E-2522 -->

* Magento Open Source では、製品の詳細 ページ **[!UICONTROL Meta Description]** フィールドが空のままになっている場合、製品説明テキストが挿入されなくなりました。 以前は、製品の説明にページビルダーまたは別のHTML編集者によって生成されたHTMLコードが含まれている場合、HTMLはメタ **[!UICONTROL Description]** フィールドに表示され、このフィールドを空のままにすることはできませんでした。

<!-- ACP2E-2308 -->

* コンフィギュレーション可能な製品 PDP ページのコンフィギュレーション可能な製品オプションは、ページ上の他のコンフィギュレーション可能な製品オプションの影響を受けなくなりました。 以前は、ドロップダウン属性を持つコンフィギュレーション可能な製品は、製品の詳細ページに製品カルーセルが存在する場合に例外をトリガーしなくなりました。 スウォッチオプションを持つ他の構成可能製品がページに存在する場合、構成可能な製品オプションは選択できませんでした。
...
* スケジュール更新によって有効にした場合、カタログルールのインデックスが正しく作成されるようになりました。 その結果、割引価格が正しく生成され、インデックスに登録されるようになりました。 以前は、2 つのカタログ ルールが使用可能で、最初のルールルールのインデックス作成操作が完了する前に最初のルールの後に 1 つのルールがアクティブ化された場合、2 番目のカタログルールに対してカタログルール割引価格が生成されませんでした。
...
* 複数のオプションを持つ構成可能プロダクトの保管操作のパフォーマンスが向上しました。 以前は、製品の保存操作中にタイムアウトが発生することがありました。 [GitHub-36928](https://github.com/magento/magento2/issues/36928)

<!-- ACP2E-1764 -->

* コンフィギュレーション可能な製品価格のストアフロント表示は、買い物客が製品オプションを選択したときに期待どおりに変化するようになりました。 [GitHub-37378](https://github.com/magento/magento2/issues/37378)

### クーポン

<!-- ACP2E-2495 -->

* 顧客あたりの使用回数が限られているクーポンコードは、以前に使用された注文が失敗した場合に2回目で使用できるようになりました。 以前は、前回の注文がキャンセルされたときにプロモーションコードはリリースされませんでした。

<!-- ACP2E-2428 -->

* スペース文字を含むクーポンコードが無効でなくなりました。 以前は、クーポンコードにスペース文字(実際のコードの前後)が含まれている場合、検証失敗していました。 [GitHub-38048](https://github.com/magento/magento2/issues/38048)

<!-- ACP2E-1551 -->

* 買い物客の使い捨てクーポンの値は、適用された注文がキャンセルされたときに期待どおりに復元されるようになりました。

<!-- ACP2E-1195 -->

* `GET V1/coupons/<couponId>` エンドポイントは、自動生成されたクーポンの場合と同様に、手動で作成されたクーポンに対して予想される完全な応答を返すようになりました。以前は、一部のフィールド( `usage_limit`、 `usage_per_customer`、 `created_at`など)が省略されていました。

### cron

<!-- ACP2E-2457 -->

* `aggregate_sales_report_bestsellers_data` cron ジョブでは、非常に大きな一時 MySQL テーブルが作成されなくなり、正常に完了しました。この cron ジョブでは、すべてのストアではなく、ストアごとにデータを一度に挿入するようになりました。 以前は、この cron ジョブを実行すると `No space left on device` エラーが発生する可能性がありました。

<!-- ACP2E-2427 -->

* Cron ジョブが不要な cron ジョブロックを取得しようとするのを防ぎ、`cron_schedule` テーブル内の保留中のレコード数を削減しました。 以前は、cron ジョブは、ジョブを実行するには遅すぎるときにロックを取得しようとしました。 `cron_schedule` の cron ジョブは、すでに予定時刻を過ぎていても、欠席均等としてマークされませんでした。Magento Open Sourceまた、エラーログにこのエラーを繰り返し表示しました: `report.WARNING: Could not acquire lock for cron job: indexer_update_all_views`。

<!-- AC-9207 -->

* cron 式のゼロ除算の致命的なエラー固定。 [GitHub-37804](https://github.com/magento/magento2/issues/37804)
...
* 非推奨の `ZendCurrency` クラスとの機能の等価性を維持するために、通貨形式の `precision` オプションのサポートを追加しました。
...
* ユーザー が管理者から 無効 URL にアクセスしようとするシナリオのエラーメッセージを改善しました。 [GitHub-35682](https://github.com/magento/magento2/issues/35682)。

<!-- ACP2E-1841 -->

* `Param $options[`通貨記号の position`]` が期待どおりに動作するようになり、通貨記号の位置のカスタマイズがサポートされるようになりました。 以前は、Zend Framework 1 から移行した後、 `toCurrency` メソッドは `position` パラメータを無視していました。 この問題は、管理インターフェイス全体で発生していました。
...
* `customer_entity_int` の `value_id` の値が正しく増分されるようになりました。 これらの属性を組み込んだエンティティを更新すると、 `insert on duplicate key update` ではなく更新クエリが実行されます。 その結果、EAV モデル内で使用される自動インクリメント列は直線的に拡大するようになりました。 以前は、挿入クエリの失敗により、列の自動インクリメントプロセスで値がスキップされていました。 [GitHub-28387](https://github.com/magento/magento2/issues/28387)

<!-- AC-8462 -->

* 不要な空白を属性 `id` から削除しました。 (このバグは Magento Open Source 2.4.6 のコード変更によって導入されました。

<!-- AC-7923 -->

* ストアストアコードがシングルストアモードのときは、ストアURLに表示されなくなりました。 [GitHub-36831](https://github.com/magento/magento2/issues/36831)
...
* スウォッチ `Helper` がリファクタリングされ、誤解を招く読み込みエイリアスが削除されました。 [GitHub-31373](https://github.com/magento/magento2/issues/31373)

<!-- AC-8708 -->

* 管理者ユーザーは、 `sitemap.xml` ファイルの削除や名前変更が期待どおりに行えるようになりました。 以前は、管理者ユーザーは管理者からファイルを削除できましたが、ファイルはファイルシステムに残りました。 [GitHub-37468](https://github.com/magento/magento2/issues/37468)

<!-- ACP2E-1747 -->

* クーポンは、適用された注文の支払いが失敗した場合に使用済みとしてフラグが付けられなくなりました。 キュー メッセージは、コンシューマーの開始時にパブリッシュされた順序で処理されるようになりました。 以前は、メッセージがキューに入れられたかどうかを確認するために最初のメッセージがキューからプルされ、後で拒否されたため、最初のメッセージが最後に処理されていました。
...
* 管理者の顧客の住所 **都道府県** フィールドに、最後に保存された値が期待どおりに保持されるようになりました。 以前は、このフィールドから値を削除した場合、Magento Open Sourceは引き続き、削除された値を **都道府県** フィールドに表示していました。 [GitHub-36846](https://github.com/magento/magento2/issues/36846)
...
* 管理者ユーザーは、ストア ID が web サイト ID と一致しない別の web サイト用にその顧客が作成された場合、管理者から新しい顧客アドレスを追加できるようになりました。 以前は、Magento Open Sourceには「`Something went wrong`」というポップアップメッセージが表示されていました。 [GitHub-36582](https://github.com/magento/magento2/issues/36582)
...
* `ProductInterface` で `related_products`、`upsell_products`、`crossell_products` をクエリする際に、フラグメント `ProductCard` を使用できるようになりました。 [GitHub-29769](https://github.com/magento/magento2/issues/29769)
...
* 配送用テーブルの料金が、テーブル料金インポートの CSV ファイルで期待どおりに更新されるようになりました。 テーブルレートのアップロードファイルは、非同期設定プロセッサの cron ジョブが実行されるまで、一時的に保存されます。 この cron ジョブは、ファイルの新しい場所を取得し、それに従って処理します。 以前は、非同期プロセッサが間違った場所でファイルを検索したため、テーブル レートは期待どおりに更新されませんでした。

<!-- ACP2E-2298 -->

* 書き出しファイルグリッドに、タイムスタンプに関係なく、エクスポートされたすべてのファイルが表示されるようになりました。 以前は、タイムスタンプが同じエクスポートされたすべてのファイルがグリッドに表示されませんでした。 [GitHub-36951](https://github.com/magento/magento2/issues/36951)
...
* スケジュールされた読み込み処理中に画像が正常に読み込まれなかった場合、Magento Open Sourceのスケジュールされた読み込みグリッドに情報エラーメッセージが表示されるようになりました。 以前は、エラーメッセージは表示されませんでした。
...
* Magento Open Sourceでは、不正な形式の HTTP リクエストがサーバーに送信されたときに表示されるエラーメッセージに、サーバーのルートへのフルパスが含まれなくなりました。 文字列のみが 2 番目の引数として `explode` メソッドに渡されるようになりました。
...
* マーチャントは、製品に対して **[!UICONTROL No Manage Stock]** 設定が有効になっている場合、注文を受け取る準備が整ったことを買い物客に通知できるようになりました。 以前は、この設定を有効にすると、Magento Open Sourceに「`Your order is not ready for pickup`」というエラーメッセージが表示されていました。
...
* 出荷元を選択する際に、買い物客が利用可能な商品の範囲内にある注文から商品数量を控除した場合に、ページの再読み込み後にMagento Open Sourceがエラーをスローしなくなりました。
...
* ストアフロントから注文されたデフォルト以外のソースが出荷用に処理される際に、デフォルトとデフォルト以外の両方のソースに、製品の販売可能数量に関する正しい情報が表示されるようになりました。 以前は、Magento Open Sourceで商品ソースの間違った在庫ステータスが表示されていました。
...
* Magento Open Sourceでは、管理者ユーザーが管理者で順序を作成した際に指定された並べ替え順に基づいて、カスタムの顧客住所属性を並べ替えるようになりました。

<!-- ACP2E-2400 -->

* マーチャントがこのページから検索を開始すると、「管理受注グリッド」ページが期待どおりに読み込まれるようになりました。 以前は、マーチャントがこのテーブルから注文を検索しようとすると、ページがフリーズし、Magento Open Sourceブラウザーコンソールに `Request-URI Too Long` エラーが表示されていました。

<!-- ACP2E-2213 -->

* `OrderRepository::get()` メソッドでデータベースから注文をロードする回数を減らすことでパフォーマンスを向上させました。以前は、このメソッドで注文が複数回読み込まれていました。 [GitHub-36636](https://github.com/magento/magento2/issues/36636)

<!-- ACP2E-1918 -->

* マーチャントは、他の注文アイテムが払い戻し可能な場合に、合計がゼロのアイテムを含む注文のクレジットメモを生成できるようになりました。 以前は、顧客注文の一部のアイテムに100%の割引がある場合、マーチャントは複数の返金を実行できませんでした。

<!-- AC-8254 -->

* 子商品と親商品間のリンクが、管理者からの子商品の再注文時に注文ページに表示されるようになりました。 [GitHub-37028](https://github.com/magento/magento2/issues/37028)

<!-- ACP2E-1341 -->

* `getInfoBlockHtml()`呼び出し中にエミュレーションが完了すると、`send()`呼び出し中にエミュレーションが開始されるようになり、エラーはログに記録されません。以前は、 `system.log` ファイルは次のエラーであふれていました: `main.ERROR: Environment emulation nesting is not allowed`。 [GitHub-35603](https://github.com/magento/magento2/issues/35603)
...
* 買い物客は、管理者から 1 回限りのカスタム価格を使用して注文が最初に生成されたときに、カスタム価格を使用して製品を注文できなくなりました。 以前は、注文にカスタム価格の商品が含まれている場合、このカスタム価格は、再注文機能を使用して注文された他の注文に適用されていました。
...
* 注文ページの 注文顧客 セクションの購入日によるフィルター機能は、期待どおりに機能するようになりました。 以前は、同じタイムゾーンがメソッドに指定されたときにタイムゾーンコンバージョンロジックがエラーをスローすると、JavaScriptエラーが発生しました。

<!-- ACP2E-1294 -->

* 管理ダッシュボードに、初回読み込み時の正しい注文統計が表示されるようになりました。 以前は、ダッシュボードに誤った注文情報が表示されていましたが、期間が更新されると正しい情報が表示されていました。

<!-- ACP2E-1762 -->

* 管理者ユーザーは、管理者からの新しい注文の作成時にストア表示を選択した後、期待どおりに注文ページにリダイレクトされるようになりました。

<!-- ACP2E-1418 -->

* `GET V1/orders/<OrderID>`エンドポイントは、シンプル製品の在庫ステータスに関係なく、注文内のコンフィギュレーション可能製品とシンプル製品の両方に関する情報を返すようになりました。管理者ユーザーは、選択した品目(製品)が在庫または販売可能な場合にのみ注文できるようになりました。 以前は、この API は在庫のある製品に関する情報のみを返していました。

<!-- AC-7901 -->

* 送料に税が含まれており無償割引後に税計算が適用されるときに出荷が有効になっている場合、クレジットメモページクラッシュしなくなりました。 以前は、Magento Open Source は `Division by zero` 例外をスローしていました。 [GitHub-36800](https://github.com/magento/magento2/issues/36800)

<!-- AC-1342 -->

* `rest/V1/orders/{id}/comments` エンドポイントを使用して、注文ステータスを指定せずに注文に関するコメント投稿しても、注文の表示に影響しなくなりました。以前は、注文の状態が `sales_order` テーブルと `sales_order_grid` テーブルで NULL としてログに記録され、[マイ オーダー] ページ にも [管理オーダー] グリッドにも注文が表示されませんでした。 [GitHub-34180](https://github.com/magento/magento2/issues/34180)

### ページビルダー

<!-- AC-8777 -->

* ページ Builder で画像のサイズ変更時に、画像が正しい縦横比を維持するようになりました。 [GitHub-37493](https://github.com/magento/magento2/issues/37493)

<!-- AC-10919 -->

* ページ Builder では、指定した最大ファイルサイズよりも大きい画像をアップロードしようとすると、必要に応じてエラーメッセージが表示されるようになりました。 以前は、Commerce は、指定された最大値よりも小さいアップロードされた画像ファイルに対してアラートポップアップ メッセージを表示していました。

<!-- ACP2E-1992 -->

* CMS ページ製品スライダーには、スライダーに含めるために選択された子製品の親製品が含まれるようになりました。 以前は、親製品はスライダーから除外されていました。
...
* Magento Open Sourceは、このエラーが発生した場合に注文の作成を防ぐことで、PayPal からの 502 エラーを正しく処理するようになりました。 以前は、PayPal の 502 エラーにもかかわらず、注文は引き続き作成されており、Magento Open Source レコードと PayPal の間で不一致が発生していました。
...
* Magento Open Sourceが支払った注文に対して **[!UICONTROL Enable Vault for Card Payments]** が有効な場合に、Braintreeが 2 番目の部分請求書を作成するとエラーがスローされなくなりました。 以前は、マーチャントが 2 つ目の請求書を作成しようとすると、Magento Open Sourceが `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid` というエラーをスローしていました。
...
* 買い物客は、Braintree Payments と、非表示の ReCAPTCHA v2 または v3 を使用したMagento Open Source 2.4.6 を使用して、チェックアウトを完了できるようになりました。 [GitHub-37241](https://github.com/magento/magento2/issues/37241)
...
* `Magento\Catalog\Model\ProductRepository`から未使用の`Magento\Catalog\Controller\Adminhtml\Product\Initialization\Helper`を削除。以前は、 `ProductRepository` このヘルパー( `Adminhtml` コントローラー)を読み込んでいました。 [GitHub-37864](https://github.com/magento/magento2/issues/37864)

<!-- AC-9127 -->

* 同梱商品に多くのオプションを追加した場合のページの読み込み速度を改善しました。 [GitHub-29409](https://github.com/magento/magento2/issues/29409)
...
* 名前による製品のアルファベット順の並べ替えが、トルコ語文字を含む名前に対して正しく機能するようになりました。

<!-- ACP2E-1854 -->

* 製品属性&#x200B;**[!UICONTROL Use in Search Results Layered Navigation]**&#x200B;プロパティは、設定が「**[!UICONTROL Yes]**」に設定されている場合のみ&#x200B;**[!UICONTROL Use in Search]**&#x200B;表示されるようになりました。

<!-- ACP2E-1870 -->

* 高度な検索価格とSKUフィルターは、高度な検索で一緒に適用すると正しく機能するようになりました。 [GitHub-37581](https://github.com/magento/magento2/issues/37581)

<!-- ACP2E-1787 -->

* 空の検索フィールドに対しては、検索は実行されなくなりました。 これにより、 `search_query` テーブルのサイズが小さくなります。 以前は、誤ったクエリや空の 検索 フィールドに対するクエリによって膨大なデータセットが返され、 `search_query` テーブルのサイズが大きくなり、 `/tmp`問題が発生していました。

<!-- ACP2E-1916 -->

* ストアフロントフィルター、ドロップダウンカスタム属性でカタログエントリを並べ替えるときに期待どおりに機能するようになりました。

<!-- ACP2E-1499 -->

* Search結果には検索期待どおりに属性の重み付けに基づいて関連する結果が表示されるようになりました。

<!-- AC-7505 -->

* Admin **[!UICONTROL Stores]** > **[!UICONTROL Attributes]** > **[!UICONTROL Product]** の `Search Weight` 属性の`Storefront Properties`入力フィールドの下の情報テキストで、可能な値が説明されるようになりました。[GitHub-36646](https://github.com/magento/magento2/issues/36675)

### 出荷

<!-- ACP2E-2515 -->

* 動的な価格属性が無効になっている場合に、バンドル商品が買い物かごに追加されたときに、カート価格ルールが期待どおりに機能するようになりました。 送料無料と正しい表送料は、動的属性が有効か無効かに関係なく適用されます。 以前は、送料無料は期待どおりに適用されませんでした。

<!-- AC-9729 -->

* Magento Open Sourceは、注文の部分出荷を作成した後、期待どおりに新しい出荷ページに移行するようになりました。 以前は、一部の出荷を作成した後は、Magento Open Sourceは、注文の残りの製品の出荷完了ページを作成していませんでした。 [GitHub-38063](https://github.com/magento/magento2/issues/38063)
...
* 管理者ユーザーがユーザーとしてログイン機能を使用した場合、複数の配送先住所を含むチェックアウト中にMagento Open Sourceが例外をスローしなくなりました。
...
* テンプレートのレンダリング中に発生するエラーのメッセージを改善しました。 Magento Open Sourceに情報メッセージが表示されるようになりました。 以前は、Magento Open Source は PHP エラーが発生するとリークHTMLが表示されていました。
...
* Mixin 設定値が空ではなくなり、読み込み位置に関係なく jQuery Mixin が機能するようになりました。 以前は、jQuery mixin はブラウザー間で一貫してロードされていませんでした。

<!-- AC-8823 -->

* ログインページの **[!UICONTROL Show Password]** チェックボックスがアクティブのときに、期待どおりにパスワードが表示されるようになりました。

<!-- ACP2E-2034 -->

* Magento Open Source で、特別価格の固定バンドル商品の正しい価格が表示されるようになりました。

<!-- AC-8285 -->

* ストアフロントのフッターテンプレートファイルの GitHub の問題リストページリンクを更新しました。 [GitHub-37214](https://github.com/magento/magento2/issues/37214)

<!-- AC-6282 -->

* 最近表示された項目ウィジェットに、グループ化された商品価格に正しい税額が表示されるようになりました。 [GitHub-35881](https://github.com/magento/magento2/issues/35881)
...
* システムメッセージの読み込みが速くなりました。 表示されているメッセージのみが読み込まれます。 以前は、すべてのメッセージがデータベースから読み込まれていたため、時間がかかりました。

<!-- ACP2E-1804 -->

* レイアウト XML で `remove=true` を設定してノードパンくずリストを削除しても、カテゴリ ページのページタイトルが削除されなくなりました。 以前は、この方法でパンくずリストを削除すると、カテゴリページからページタイトルが削除されていました。
...
* 単一引用符を含むようこそメッセージは、コンソールエラーなしでストアフロントで正しくレンダリングされるようになりました。 以前は、ウェルカムメッセージに単一引用符が含まれている場合、ストアフロントのレンダリング中にウェルカムメッセージが読み込まれませんでした。 Magento Open Sourceコンソールエラーも表示されました。

<!-- ACP2E-1369 -->

* 管理オーダーページでコメント履歴と返品の間の表示をすばやく変更しようとしたときに、Magento Open Sourceエラーがスローされなくなりました。 以前は、Magento Open Source で表示された一般的なエラー `A technical problem with the server created an error`。
...
* 正規URLタグには、カテゴリURLキープロパティの更新後に期待どおりに変更が反映されるようになりました。 以前は、古いカテゴリ URL が使用されていましたが、これにより、新しい URL へのリダイレクトがすぐにトリガーされました。
...
* 階層価格と様々な属性セットを使用して製品を更新する非同期の一括 REST API 操作が正常に完了しました。 以前は、異なる属性セットを持つ商品は、この方法で更新できませんでしたが、Magento Open Sourceに「`The product was unable to be saved. Please try again`」というエラーが表示されていました。
...
* Magento Open Sourceは、product media API 応答で `content` 属性（base64 画像コード） `GET V1/products/sku/media` 正常に返すようになりました。 `content` 属性は、以前は応答に表示されていませんでした。

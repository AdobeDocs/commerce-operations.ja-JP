...
* **Vanish キャッシュ 7.3 のサポート**. このリリースは Vanish Cache 7.3 の最新バージョンと互換性があります。互換性は 6.0.x および 7.2.x バージョンに残りますが、Magento Open Source2.4.7-beta1 は Vanish Cache バージョン 7.3 またはバージョン 6.0 LTS でのみ使用することをお勧めします。

* **RabbitMQ 3.11 のサポート**. このリリースは、RabbitMQ 3.11 の最新バージョンと互換性があります。互換性は引き続きRabbitMQ 3.9 （2023 年 8 月までサポート）と互換性がありますが、RabbitMQ 3.11 とのみでMagento Open Source2.4.7-beta1 を使用することをお勧めします。
...
* **新しいGraphQL Resolver キャッシュタイプ** 次を含む：
   ...
   * CMS はブロックします。 これは、 `Magento\CmsGraphQl\Model\Resolver\Blocks::resolve`. <!-- B2B-2659 -->
...
* ストア ID が Web サイト ID と一致しない別の Web サイトに対して、その顧客が作成された場合、管理者ユーザーは、その顧客が管理者から新しい顧客アドレスを追加できるようになりました。 以前は、Magento Open Sourceが次のポップアップメッセージを表示していました。 `Something went wrong`. [GitHub-36582](https://github.com/magento/magento2/issues/36582)
...
* この `addProductsToCart` クエリがギフトカードを買い物かごに正常に追加しました。 以前は、このクエリではギフトカードの受信者名が想定されていましたが、送信者名が代わりに返されていました。 Magento Open Sourceが次のエラーをスローしました： `Please specify a recipient name`. [GitHub-36665](https://github.com/magento/magento2/issues/36665)
...
* `product` 指定した共有カタログに割り当てられた製品の集計のみを返すようになりました。
...
* ページビルダーのテキストエディター間でのテキストのコピー&amp;ペーストが期待どおりに動作するようになりました。 以前は、テキストを貼り付けようとしたページは保存できず、Magento Open Sourceがこのコンソールエラーを表示していました。 `[ERROR] Page Builder was rendering for 5 seconds without releasing locks.`.
...
* Magento Open Sourceで、Braintreeが支払った注文の 2 番目の部分請求書を作成する際に、 **[!UICONTROL Enable Vault for Card Payments]** が有効になっている。 以前は、商人が 2 番目の請求書を作成しようとすると、Magento Open Sourceは次のエラーをスローしていました。 `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid`.
...
* コンソールエラーなしで、一重引用符を含むようこそメッセージがストアフロントに正しくレンダリングされるようになりました。 以前は、ようこそメッセージに一重引用符が含まれていた場合、ストアフロントレンダリング中にようこそメッセージが読み込まれませんでした。 Magento Open Sourceにコンソールエラーも表示されました。
...
* Magento Open Sourceが `content` の属性（base64 イメージコード） `GET V1/products/sku/media` 製品メディア API 応答。 この `content` 属性は、以前は応答に表示されていませんでした。

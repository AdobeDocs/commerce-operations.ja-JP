...
ACSD-54660 パッチは、[!DNL GraphQL]の顧客注文を`sort_field`と`sort_direction`で並べ替えるために新しい入力属性`sort`が追加された問題を修正します。 このパッチは、[[!DNL Quality Patches Tool (QPT)]](/help/tools/quality-patches-tool/quality-patches-tool-to-self-serve-quality-patches.md) 1.1.39がインストールされている場合に利用できます。 パッチ IDはACSD-54660です。 この問題は、Adobe Commerce 2.4.6で修正される予定です。

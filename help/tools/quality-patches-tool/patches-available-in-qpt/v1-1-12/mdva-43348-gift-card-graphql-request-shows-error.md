...
description: MDVA-43348 パッチは、「gift_card_options」に「uid」が含まれている場合にギフトカードGraphQL リクエストでエラーが表示される問題を修正します。 このパッチは、[Quality Patches Tool （QPT） ] （https://experienceleague.adobe.com/en/docs/commerce-operations/tools/quality-patches-tool/quality-patches-tool-to-self-serve-quality-patches） 1.1.12がインストールされている場合に利用できます。 パッチ IDはMDVA-43348です。 この問題は、Adobe Commerce 2.4.5で修正される予定です。

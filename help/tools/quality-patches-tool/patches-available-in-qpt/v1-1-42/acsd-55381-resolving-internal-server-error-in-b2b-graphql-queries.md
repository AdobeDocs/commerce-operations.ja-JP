...
ACSD-55381 パッチは、B2B要件リストの`configurable_product_option_uid`および`configurable_product_option_value_uid` フィールドに対するGraphQL クエリ中に内部サーバーエラーが発生する問題を修正します。 このパッチは、[[!DNL Quality Patches Tool (QPT)]](/help/tools/quality-patches-tool/quality-patches-tool-to-self-serve-quality-patches.md) 1.1.42がインストールされている場合に利用できます。 パッチ IDはACSD-55381です。 この問題は、Adobe Commerce 2.4.7で修正される予定です。

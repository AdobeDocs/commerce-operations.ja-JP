...
* **ACSD-67091** （Adobe CommerceおよびMagento Open Source >=2.4.8 &lt;2.4.9 の場合） – データ量に基づいて 2 つの削除戦略を実装することで、カタログルールの製品インデックスのクリーンアップを確実に行うための最大書き込みセットサイズのエラーを修正しました。
* **ACSD-67370** （Adobe CommerceおよびMagento Open Source >=2.4.7 &lt;2.4.9） – PDP/PLP のバンドル商品や、複数通貨店舗の買い物かごページで誤った価格が表示される複数の問題を修正しました。
* **ACSD-68410** （Adobe Commerceの場合、B2B >=1.3.3 &lt;1.5.3） – 交渉可能な見積もりに対して注文を行うと、見積もりに買い物かご行が誤って追加または結合される問題を修正します。 交渉可能な見積もりチェックアウトの最後の手順を終了した後、製品が買い物かごに正しく追加されるようになりました。
* **ACSD-69086** （Adobe CommerceおよびMagento Open Source >=2.4.7 &lt;2.4.8 の場合） – cron ジョブで変更ログテーブルをクリアできず、大量のデータを処理する際に Galera クラスターがクラッシュする問題を修正しました。
* **ACSD-69115** （Adobe Commerce >=2.4.4 &lt;2.4.9 の場合） – デフォルト以外の web サイトに割り当てられたお客様の買い物かごを管理する際に、管理者ユーザーに買い物かごのエラーが表示されない問題を修正しました。
* **ACSD-69129** （Adobe CommerceおよびMagento Open Source >=2.4.5 &lt;2.4.7 || >=2.4.8 &lt;2.4.9） – デフォルトのベース web サイトを削除し、セカンダリ web サイトをデフォルトとして使用すると、REST API を使用してセカンダリ web サイトの階層価格を更新しようとするとエラーが発生する問題を修正しました。
* **ACSD-69203** （Adobe CommerceおよびMagento Open Source >=2.4.8 &lt;2.4.9 の場合） – カテゴリ条件に複数のカテゴリがリストされている場合に、製品リストウィジェットが誤った結果を返す問題を修正しました。
* **ACSD-69261** （Adobe CommerceおよびMagento Open Source >=2.4.4 &lt;2.4.9 の場合） - 1 顧客あたりの 1 回限りの使用に設定された買い物かご価格ルールクーポンが、部分的な請求書および残りの数量のキャンセルシナリオでの `times_used` 属性の誤った処理によって複数回再利用された問題を修正しました。
* **ACSD-69308** （Adobe CommerceおよびMagento Open Source >=2.4.4 &lt;2.4.9 の場合） – Web サイトレベルでのみ設定された場合（「すべてのストアビュー」では `special_price` く）、カタログ価格ルールが適用されない問題を修正しました。 修正後、カタログの価格ルールは、最初に web サイトのデフォルトストアを確認することで、正しく適用されます。
* **ACSD-69319** （Adobe CommerceおよびMagento Open Source >=2.4.7 &lt;2.4.9 の場合） – 子商品がカスタムソースで在庫されていた場合に、バンドル価格のインデックスが正しく作成されなかった問題を修正しました。
* **ACSD-69325** （Adobe CommerceおよびMagento Open Source >=2.4.7 &lt;2.4.9 の場合） - SKU ケースを変更すると、ストアフロントで商品が在庫切れに表示される問題を修正しました。
* **ACSD-69331** （Adobe CommerceおよびMagento Open Source >=2.4.5 &lt;2.4.9 の場合） – メディアギャラリーのコンテンツ作成者が `create_folder` 権限だけではフォルダーを作成できない問題を修正しました。 修正後、期待どおりにフォルダーを作成できます。
* **ACSD-69333** （Adobe Commerce >=2.4.7 &lt;2.4.9 の場合） – スケジュール済みのアクティブなアップデートを使用した商品の SKU が変更できる問題を修正しました。 修正後は、アクティブな更新中に SKU の変更が禁止され、保存が失敗して明確なエラーが表示され、管理者の SKU フィールドが無効になります。 これにより、ステージングロールバック中に SKU の変更によって発生する MSI インベントリの不整合を防止できます。
* **ACSD-69541** （Adobe CommerceおよびMagento Open Source >=2.4.4 &lt;2.4.9 の場合） – 管理画面の商品の数量を買い物かごに既に存在する数量よりも減らすと、GraphQLを使用してその買い物かごの商品数量を編集できなくなる問題を修正しました。
* 更新されたバージョン：**ACSD-46541**、**ACSD-53750**、**ACSD-66404**
* 交換後のパッチ：**ACSD-66404**、**ACSD-68499**
...
* **ACSD-52606** （Adobe CommerceおよびMagento Open Source >=2.4.0 &lt;2.4.7 の場合） – 「注文を受け取る準備ができていません *というエラーメッセージがユーザーが* をクリックすると表示される問題を修正しました **[!UICONTROL Notify Order is Ready for Pickup]**。
...
* **ACSD-52041** （Adobe CommerceおよびMagento Open Source >=2.4.4 &lt;2.4.7 の場合） – ロックを解除せずに 5 秒間レンダリングされ *[エラーメッセージ「]ERROR[!DNL Page Builder]」の問題を修正しました。* で編集したコンテンツを保存すると、Chrome ブラウザーに [!DNL Page Builder] が表示されます。
...
* **MDVA-19391** （*Adobe Commerce >=2.3.0 &lt;2.3.5* の場合） – `analytics_collect_data` テーブルの説明レコードが NULL であることが原因で `catalog_category_entity_text` がエラーをスローする問題を修正しました。
* **MDVA-20376** （*Adobe Commerce >=2.3.2 &lt;2.3.4* の場合） – 注文後にログインした顧客の *ージで、* customerId = 1 のエンティティがありません `exception.log` エラーの問題を修正しました。

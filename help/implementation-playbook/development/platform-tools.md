...
この [Adobe Commerce セキュリティスキャンツール](https://docs.magento.com/user-guide/magento/security-scan.html) を使用すると、ストアの web サイトを定期的に監視し、既知のセキュリティ リスク、マルウェア、および古いソフトウェアの更新を受け取ることができます。 通常は、ユーザー受け入れテスト（UAT）を開始する際に、このツールの使用を開始します。 Adobe Commerce セキュリティスキャンツールは、Adobe Commerceのすべての実装およびバージョンで無料で使用でき、他にも CI/CD プロセス中や各リリース前に使用できる選択肢があります。

...
必ずを設定してください。 [PHPMD と PHPCS](https://developer.adobe.com/commerce/php/best-practices/phpstorm/code-inspection/) を実行する [コーディング基準](https://github.com/magento/magento-coding-standard) cli のツール（後述） 重複する部分もありますが、両方とも一意のテストを実施しています。
...
- 設定は常に可能な限り低いレベル（ストア表示レベル、web サイトレベル、グローバルレベル）で取得されますか？ 設定の取得は、の範囲の定義と一致する必要があります `system.xml` ファイル。
...
- 見つかりますか？ `cachable="false"` 属性？ 正しく適用されていますか？
...
プライバシーと [GDPR](../../../security-and-compliance/privacy/gdpr.md) 手動で行います。
...
- データベーススキーマ `whitelist.json` – 次の CLI コマンドを実行して、 `db_schema_whitelist.json` ファイルは追加または変更されません。

  ```bash
  bin/magento setup:db-declaration:generate-whitelist --module-name[=MODULE-NAME]
  ```

- Composer の検証 – を検証します。 `composer.json` を含むディレクトリで次の CLI コマンドを実行して、ファイルに `composer.json` ファイル。

  ```bash
  composer validate
  ```

- コーディング標準 – コーディング標準ツールをインストールして実行し、モジュールに対して実行します。 次のファイルは、を入力して任意の場所で実行できるようにする方法を示しています。 `mcs ./app/code/Vendor/Module/`.

...
- クラウドプロジェクトで[ECE ツールパッケージ v2002.1.12](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/dev-tools/ece-tools/update-package)以降が使用されていることを確認します。
...
>[!CAUTION]
>
>`.magento.env.yaml`設定を更新する場合は、プロジェクトのValkey サービス以外のキャッシュエンドポイントを意図的に指定しない限り、`server`または`port`を上書きしないでください。 ECE ツール パッケージは、これらの値をValkey サービス関係から自動的に導き出します。 誤った値で上書きすると、デプロイメントがキャッシュ接続エラーで失敗します。

`VALKEY_BACKEND`のデプロイメント変数を`symfony_l2`に設定すると、`default` フロントエンドと`stale_cache_enabled` フロントエンドを含むValkey サービス接続の詳細から完全なL2 キャッシュ設定が自動的に構築され、キャッシュ可能なタイプ（`layout`、`block_html`、`full_page`、`translate`など）が既に古い対応フロントエンドにマッピングされています。 `symfony_l2`を使用するために`CACHE_CONFIGURATION`を定義する必要はありません。

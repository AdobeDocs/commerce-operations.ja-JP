...
環境の設定の詳細については、「_Commerce on Cloud Infrastructure Guide_」の「[`VALKEY_BACKEND`](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/configure/env/stage/variables-deploy#valkey_backend) コンフィギュレーション変数」を参照してください。
...
Adobe Commerce 2.4.9以降では、`symfony_l2` キャッシュバックエンドがサポートされています。 クラウドインフラストラクチャ上のAdobe Commerceで、このバックエンドを設定するのは、`.magento.env.yaml`の`symfony_l2`をサポートする`ece-tools` バージョンをプロジェクトで使用した後のみです。

`symfony_l2` バックエンドは、Adobe CommerceがL1およびL2 キャッシュ動作の管理に使用するキャッシュ実装です。 RedisまたはValkeyをリモートキャッシュサービスとして置き換えることはありません。 Adobe Commerce 2.4.9の場合は、Valkeyをリモートバックエンドとして`symfony_l2`を設定します。

>[!IMPORTANT]
>
>プロジェクトで`ece-tools`のサポートが利用可能になるまで、`app/etc/env.php`で`symfony_l2`をクラウドインフラストラクチャ上のAdobe Commerceの永続的な設定として手動で設定しないでください。 展開では、手動の`env.php`変更を上書きできます。 `ece-tools`が`symfony_l2`を適用しない場合、Commerceはファイルベースのキャッシュにフォールバックできます。 このフォールバックにより、ディスク I/Oが増加し、マルチノード環境でのファイルシステムのレプリケーションのオーバーヘッドが増加し、パフォーマンスが低下する可能性があります。

`ece-tools`のサポートが利用可能な場合は、Valkey バックエンド変数を`symfony_l2`に設定し、`CACHE_CONFIGURATION`でL2 バックエンドオプションを定義します。
...
`symfony_l2`で選択したキャッシュタイプの古いキャッシュを有効にするには、`use_stale_cache: true`で2番目のフロントエンドを定義し、選択したキャッシュタイプをそのフロントエンドにマッピングします。 各フロントエンドには、明確なローカルキャッシュディレクトリと永続的なIDを使用します。
...
- アプリケーションレベルで`cleanup_percentage` プロパティを調整して、他のサービスで利用可能なストレージ使用量と空きメモリを制限します。キャッシュ構成グループ `cache/frontend/default/backend_options/cleanup_percentage`の配備構成で構成を調整できます。

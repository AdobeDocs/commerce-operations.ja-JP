...
| 反射 | 反射 | Webapi モジュールと顧客モジュール間の依存関係を削除します。 |
...
このコマンドを使用すると、すべてのキャッシュタイプまたは指定したキャッシュタイプのみを有効または無効にできます。 キャッシュタイプを無効にすると、キャッシュをフラッシュしなくても変更の結果が表示されるので、開発時に役立ちます。ただし、キャッシュタイプを無効にすると、パフォーマンスに悪影響を与えます。
...
省略する場合 `[type]` すべてのキャッシュタイプを同時に有効または無効にします。 The `type` オプションは、スペースで区切られたキャッシュタイプのリストです。

<!-- `--bootstrap=` is a URL-encoded associative array of Commerce [application bootstrap parameters](../bootstrap/set-parameters.md#bootstrap-parameters) and values. -->

キャッシュのタイプとステータスをリストするには、次の手順に従います。
...
>[!INFO]
>
>バージョン 2.3.4 以降、Commerce はすべてのシステム EAV 属性を取得時にキャッシュします。 EAV 属性をこの方法でキャッシュすると、DB への挿入/選択要求の量が少なくなるので、パフォーマンスが向上します。 ただし、キャッシュのネットワークサイズも増加します。 開発者は、 `bin/magento config:set dev/caching/cache_user_defined_attributes 1` コマンドを使用します。 これは、 [開発者モード](../bootstrap/application-modes.md) 設定によって **ストア** /設定 **設定** > **詳細** > **開発者** > **キャッシュ設定** > **ユーザー定義属性をキャッシュ** から **はい**.
...
キャッシュから古い項目をパージするには、次の手順を実行します。 _clean_ または _フラッシュ_ キャッシュタイプ：

...
このトピックでは、Commerce Enterprise B2B 拡張機能の設定パスを示します。 The [`magento app:config:dump` command](../cli/export-configuration.md) は、これらの値を共有設定ファイルに書き込みます。 `app/etc/config.php`（ソース管理下に置く必要があります）
...
| 名前 | 設定パス | 暗号化？ | システム固有？ | 敏感？ |
|--------------|--------------|--------------|--------------|--------------|
| ストアフロントからの会社の登録を許可 | `company/general/allow_company_registration` | | | |
...
| 会社クレジット変更メールコピーの送付先 | `company/email/company_credit_change_copy` | | | |

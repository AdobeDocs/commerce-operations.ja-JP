...
の設定 **frontName** を使用して Commerce 管理者 url に追加します。 `backend` env.php のノード。
...
様々な情報の詳細 [キャッシュタイプ](../cli/manage-cache.md).
...
コマースアプリケーションの cron ジョブを有効または無効にします。 デフォルトでは、cron ジョブが有効になっています。 無効にするには、 `cron` 設定を `env.php` ファイルを開き、値を `0`.
...
このノードを使用して、Commerce は `env.php` ファイルを編集してから、管理のフィールドを無効にします。

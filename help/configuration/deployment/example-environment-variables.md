...
- デフォルトの電子メールドメイン（システム固有） **ストア** /設定/ **設定** /顧客 > **顧客設定** > **新しいアカウントオプションの作成**
...
1. をクリア **デフォルトを使用** 横のチェックボックス **メール送信先** フィールドに入力します。
1. 「 」フィールドに電子メールアドレスを入力します。
1. クリック **設定を保存**.
1. 左側のウィンドウで、「顧客」/ **顧客設定**.
1. 右側のウィンドウで、を展開します。 **新しいアカウントオプションの作成**.
1. をクリア **システム値を使用** 横のチェックボックス **デフォルトの電子メールドメイン** フィールドに入力します。
1. 「 」フィールドにドメイン名を入力します。
1. クリック **設定を保存**.
...
管理で設定を変更したら、この節で説明するように共有設定をファイルに書き込みます。
...
の値 `<SCOPE>` が `CONFIG__DEFAULT__` グローバルスコープまたは `CONFIG__WEBSITES__<WEBSITE CODE>` （web サイトの範囲の場合）

の値を検索するには、以下を実行します。 `<SYSTEM__VARIABLE__NAME>`、 `/` 文字に 2 個のアンダースコアを付けます。
...
1. の任意の場所 `index.php`で、次のような変数に値を設定します。

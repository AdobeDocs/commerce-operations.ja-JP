...
ここでは、コマースの開発と実稼動システムに焦点を当てます。 Commerce をインストールする場合は、 [プリインストールの所有権と権限を設定する](../../installation/prerequisites/file-system/configure-permissions.md).
...
- **2 人のユーザー** — 独自の Commerce サーバーを実行する場合は、2 人のユーザーをお勧めします。1 人はファイルを転送し、1 つはコマンドラインユーティリティを実行し、もう 1 人は Web サーバーソフトウェア用の別のユーザーです。 可能な場合は、より安全なので、この方が望ましいです。
...
サイトを実稼動環境にデプロイする準備が整ったら、セキュリティを強化するために、次のディレクトリにあるファイルから書き込みアクセス権を削除する必要があります。
...
Web サーバー・ユーザーのグループからファイルおよびディレクトリへの書き込み権限を削除するには、次の手順に従います。
...
- The **web サーバーユーザー**:Admin とストアフロントを実行します。

  Linux システムは通常、このユーザーのシェルを提供しません。Commerce サーバーに Web サーバーユーザーとしてログインしたり、Web サーバーユーザーに切り替えたりすることはできません。

- The **コマンドラインユーザ**：またはに切り替えて Commerce Server にログインします。
...
これらのユーザーは同じファイルにアクセスする必要があるので、 [共有グループ](../../installation/prerequisites/file-system/configure-permissions.md#about-the-shared-group) 彼ら二人が属する 以下の手順は、既に実行済みであることを前提としています。

次のセクションのいずれかを参照してください。

- 2 つのファイル・システムの所有者（デベロッパー・モードまたはデフォルト・モード）
...
を設定します。 [`setgid`](https://linuxg.net/how-to-set-the-setuid-and-setgid-bit-for-files-in-linux-and-unix/) ディレクトリ上のビットの場合、権限は常に親ディレクトリから継承されます。

>[!INFO]
>
>`setgid` はディレクトリにのみ適用されます。 _not_ をファイルに追加します。

さらに、ディレクトリは Web サーバーグループで書き込み可能である必要があります。 これらのディレクトリにコンテンツが存在する可能性があるので、権限を再帰的に追加します。

#### 権限の設定および `setgid`

次の手順で `setgid` および開発者モードの権限：

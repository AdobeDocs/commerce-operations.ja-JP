...
| session-save-valkey-db | データベース | データ損失から保護するために推奨される一意のValkey データベース番号。<br><br>**重要**: Valkeyを複数のタイプのキャッシュに使用する場合、データベース番号は異なる必要があります。 既定のキャッシュ データベース番号を`0`に、ページ キャッシュ データベース番号を`1`に、セッション ストレージ データベース番号を`2`に割り当てることをお勧めします。 | 0 |

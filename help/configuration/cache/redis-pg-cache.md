...
コマンドを実行して Redis キャッシュを設定すると、Commerce Environment Configuration （`<Commerce-install-dir>app/etc/env.php`）が更新されます。
...
ここでは、デフォルトで無効になっているオプションの設定を有効にする方法について説明します。
...
`allow_parallel_generation` オプションを有効にすることで、ロックを待たずに済みます。

Adobeこのオプションはデフォルトで無効になっているので、多数の設定またはブロックを持つまで無効にすることをお勧めします。
...
このオプションはフラグなので、コマンドを使用して無効にすることはできません。 設定値を手動で `false` に設定する必要があります。

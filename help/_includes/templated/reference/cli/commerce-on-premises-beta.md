...
このリファレンスには、`bin/magento` のコマンド ライン ツールから使用できる 147 のコマンドが含まれています。
...
インデックスチェックを含めます。 警告：パフォーマンスが低下する可能性があります。 [true\|false]
...
イベントプロバイダーの ID。 このオプションを使用しない場合、システム構成で設定されたイベント プロバイダの情報が返されます。
...
イベントプロバイダーの ID。 このオプションを指定すると、指定した ID でApp Builder プロジェクトのイベント登録データがフィルタリングされます。
...
New Relicでデプロイメントマーカーを作成します（v2 REST および NerdGraph の両方をサポート）。
...
このデプロイメントの Git コミットハッシュ （NerdGraph のみ）

- 値を受け入れる

#### `--deep-link`, `-d`

デプロイメントの詳細へのディープリンク （NerdGraph のみ）

- 値を受け入れる

#### `--group-id`, `-g`

デプロイメントを整理するためのグループ ID （NerdGraph のみ）
...
有効な圧縮ライブラリ。 値：gzip （デフォルト）、lzf、lz4、snappy

- 値が必要です

#### `--session-save-valkey-log-level`

有効なログレベル。 値：0 （最小の詳細） ～ 7 （最大の詳細）
...
Valkey ロックを無効にします。 値：false （デフォルト）、true
...
Valkey Sentinel がマスターを確認します。 値：false （デフォルト）、true

- 値が必要です

#### `--session-save-valkey-sentinel-connect-retries`

Valkey Sentinel 接続の再試行。
...
使用するシリアライザ （igbinary は PHP のシリアライズより 70% 高速、58% 小さい）

...
---
# bin/magento （Adobe Commerce オンプレミス 2.4.9-beta1）
...
このリファレンスには、`bin/magento` コマンドラインツールを通じて利用できる147のコマンドが含まれています。
...
完全なコマンド名の代わりにショートカットを使用して、`bin/magento`個のCLI コマンドを呼び出すことができます。 例えば、`bin/magento s:up`、`bin/magento s:upg`を使用して`bin/magento setup:upgrade`を呼び出すことができます。 任意のCLI コマンドでショートカットを使用する方法については、[&#x200B; ショートカット構文](https://symfony.com/doc/current/components/console/usage.html#shortcut-syntax)を参照してください。
...
索引チェックを含めます。 警告：パフォーマンス低下の原因となる可能性があります。 [true\|false]
...
イベントプロバイダーのID。 このオプションを使用しない場合、システム設定で設定されたイベントプロバイダーの情報が返されます。
...
イベントプロバイダーのID。 このオプションを指定すると、App Builder プロジェクトのイベント登録データが、指定されたIDでフィルタリングされます。
...
New Relicでのデプロイメントマーカーの作成（v2 RESTとNerdGraphの両方をサポート）
...
このデプロイメントのGit コミット ハッシュ （NerdGraphのみ）

- 値を受け入れる

#### `--deep-link`, `-d`

デプロイメントの詳細へのディープリンク（NerdGraphのみ）

- 値を受け入れる

#### `--group-id`, `-g`

デプロイメントを整理するためのグループ ID （NerdGraphのみ）
...
推奨される領域（グローバル、adminhtmlなど） デフォルトはグローバルです。
...
サービスコネクタで設定されたマーチャントの新しいプロジェクトを初期化します
...
サービスコネクタで設定されたマーチャントのプロジェクト情報を一覧表示します
...
Valkey圧縮ライブラリ。 値：gzip （デフォルト）、lzf、lz4、snappy

- 値が必要です

#### `--session-save-valkey-log-level`

バルキーログレベル。 値：0 （最小冗長）から7 （最も冗長）
...
Valkeyはロックを無効にします。 値：false （デフォルト）、true

- 値が必要です

#### `--session-save-valkey-min-lifetime`

Valkey min session lifetime （秒単位）

- 値が必要です

#### `--session-save-valkey-max-lifetime`

Valkey max session lifetime （秒単位）
...
Valkey Sentinel サーバー、コンマ区切り

- 値が必要です

#### `--session-save-valkey-sentinel-verify-master`

Valkey Sentinelはmasterを検証します。 値：false （デフォルト）、true

- 値が必要です

#### `--session-save-valkey-sentinel-connect-retries`

Valkey Sentinel connectは再試行します。
...
使用するシリアライザー（igbinaryはPHPのシリアライズよりも70%高速、58%小さい）

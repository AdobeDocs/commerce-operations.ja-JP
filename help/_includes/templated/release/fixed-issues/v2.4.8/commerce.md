...
* _ACP2E-3486_：製品 RestAPI の日付と時刻の属性にデフォルト値が設定されない
   * _修正ノート_:RestAPI を介して日付および日時属性のデフォルト値が正しく設定されるようになりました
...
* _AC-11718_:URL が大文字の場合のリダイレクトループ
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/38539>
* _AC-11755_:ゲストアカウントのミドルネームが保存されていません
   * _修正注_:システムはチェックアウト中にゲストアカウントのミドルネームを正しく保存し、電子メールテンプレートでアクセスできるようになりました。 以前は、ミドルネームは見積もりテーブルに保存されず、ゲストアカウントの電子メールテンプレートではアクセスできませんでした。
   * _GitHub の問題_: <https://github.com/magento/magento2/issues/38593>
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/39067>
* _AC-11919_：管理者：ページアクションのボタンが右ではなく左にフローティングされる
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/44cef3a9>
...
* _AC-13000_:ログイン顧客オプトインチェックボックスが翻訳できません
...
* _AC-6071_:顧客はログインしていますが、フロントエンドに 404 エラーが表示されます。
...
* _AC-10705_:[ 問題 ] 「データを再読み込み」ボタンに対する権限チェックを追加
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/38279>
* _AC-11427_: [問題] マーケティングルールの属性のラベルに一貫性がありません
...
* _AC-11588_：置き換え動作で製品を読み込む際に、データの検証が成功し、「読み込み」ボタンが表示される
   * _修正ノート_:システムはデータを正しく検証し、製品のインポートプロセス中に「置換」動作で「インポート」ボタンを非表示にし、意図しないデータの置換を防ぐようになりました。 以前は、システムがデータを誤って検証し、「インポート」ボタンを表示していたため、データの不整合が発生する可能性があります。
...
* _ACP2E-3536_：管理者から注文する際に、配送条件が適用されない買い物かごルール
   * _修正注意_:以前は、買い物かご価格ルールにクーポンに配送方法割引がある場合、管理UIから適用することはできませんでした。 この修正を適用すると、特定の配送方法のクーポンを含む買い物かご価格ルール割引が管理者UIから正常に適用されます。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/a52ff98f>、 <https://github.com/magento/inventory/commit/11ce816b>
* _ACP2E-3559_:SWATCH で [フレッシュ] 16 進コードが正しく更新されません
   * _修正注意_: ビジュアル スウォッチのカラー ピッカーで ユーザー によって手動で入力された 16 進コードがシステムによって変更されなくなりました。 以前は、カラーモデル間の変換エラーが原因で、特定の 16 進数コードにわずかな調整が生じていました。
...
* _ACP2E-2708_:制限付き管理者ユーザーとして許可された Web サイトでカテゴリ製品の位置を変更できません
   * _修正注_:制限付き管理ユーザー、制限付きWebサイトに割り当てられたルートカテゴリの下に含まれるカテゴリの下に製品を追加および並べ替えることができます。

### 管理者UI, 支払い/支払い方法, 注文

* _AC-13520_:スマートボタンの注文後、トランザクション認証がトランザクションTab PayPal表示されません。
...
* _ACP2E-2519_：のクーポンコード数が更新されません   複数配送で注文した場合の、「クーポンコードを管理」タブの「使用時間」列。
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/4745100c>

### Admin UI、Staging &amp; プレビュー
...
* _ACP2E-3080_：管理者の注文製品レポートの日付範囲の表示に関する問題。
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/6f4805f8>
...
* _ACP2E-3302_: [クラウド] クーポン使用レポートの不正な計算
   * _修正_: クーポン レポート グリッドの売上合計は、「割引税補償量」と「配送割引税補償量」の両方を組み込んで正確に計算されるようになりました。 以前は、これらの金額が計算に含まれていなかったため、売上合計と販売注文データの間に不一致が生じていました。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/d75cff27>
...
* _ACP2E-3067_:Magentoは、ほとんどのNew Relic cron トランザクションを破棄します#34108
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/35b1b1da>
...
* _AC-10826_：ストアフロントバンドルチェックボックス検証エラーメッセージ数が 1 を超える
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/3ea26621>
* _AC-13321_:Magento 例外がテストケースに関連して何らかの順序でスローされます
   * _修正ノート_:システムはさまざまなテストケースで「sendGuestPaymentInformation」ステップを正しく処理するようになり、Magento例外がスローされなくなりました。 以前は、これらの例外は支払い方法がnullであるために発生し、いくつかのテストケースで失敗を引き起こしていました。
...
* _AC-11876_:[ 問題 ] 2.4.7 でセールスルールのリグレッションが発生
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/0574ac23>
* _AC-11914_: [発行] 販売ルールカート固定計算:割引額が正しくない
   * _修正注_: 固定金額の販売ルールの割引額が買い物かご正しく計算されるようになり、買い物かご品目の変更に関係なく正確な割引が適用されます。 以前は、品目が変更されたときに割引額が誤って変動買い物かご可能性があり、予想よりも大幅に大きな割引が発生することがありました。
...
* _ACP2E-3004_：ゲストによる注文フォームを使用して顧客の注文を並べ替えると、買い物かごが空になる
   * _注意を修正_：以前は、「注文と返品」ページを通じて並べ替えを配置する際に、顧客はログインページにリダイレクトされていました。 この修正が適用されると、登録された顧客は、並べ替えを配置する際に、買い物かごの表示ページに正しくリダイレクトされます。 フローはゲスト顧客と同じように機能します。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/6a185204>
* _ACP2E-3025_:限定的な役割を持つ管理者ユーザーが買い物かごを表示できないリソース
   * _修正ノート_:以前は、制限付き管理者は、関連するWebサイトの管理パネルから放棄された買い物かごを見ることができませんでした。 この修正を適用すると、制限付き管理者は管理パネルから放棄された買い物かごを確認できます。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/d1f7dc95>
* _ACP2E-3176_: [クラウド] クイックオーダー大量のSKUパフォーマンス
...
* _ACP2E-3296_:姓/名に入力されたメールにチェックアウト注文の確認メールを送信
...
* _ACP2E-3402_：チェックアウトの配送先住所フォームが間違った住所で更新される
   * _修正点_:shippingAddressFromData は、web サイトごとのローカルストレージに保存されるようになりました。 以前は、URLでストアコードが使用され、同じゲストセッション中に複数のウェブサイトからチェックアウトが開始された場合、チェックアウト中に間違ったウェブサイトの住所が配送先住所フォームに自動入力されることがありました。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/078c387e>
* _ACP2E-3405_: [クラウド] Searchが有効になっている場合、チェックアウトで選択した請求アドレスアドレス保持されません。
   * _修正ノート_:住所検索が有効になっている場合、チェックアウト支払いページは選択した請求住所を保持するようになりました。 以前は、「顧客アドレス数字の制限」が1に設定されていて、顧客に複数のアドレスがある場合、ページをリロードすると選択した請求アドレスが消えていました。
* _ACP2E-3407_: ギフトカード商品 |買い物かごの結合はギフトカードを統合しています
...
* _AC-10910_: cron_schedule データベーステーブルのクリーンアップで、既存のジョブ以外のジョブがクリーンアップされない
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/38693>
* _AC-10953_:階層価格が選定可能製品から削除されていません
   * _修正ノート_: 製品は、単純な製品から選定可能製品に変換されるときに、その階層価格が正しく削除され、フロントエンドに正確な価格が表示されるようになりました。 以前は、シンプルな製品から構成可能な製品に製品が変換された際に、構成可能な製品の階層価格が削除されていなかったので、表示される価格に不一致がありました。
   * _GitHub の問題_:<https://github.com/magento/magento2/issues/38390>
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/pull/38427>
* _AC-11804_:カテゴリー 説明 デフォルト以外のストアビューで WYSIWYG が空である
   * _修正ノート_:ストア表示レベルでカテゴリを編集するときに、システムはWYSIWYG編集者にカテゴリの説明を正しく保存して表示するようになりました。 以前は、ストア表示レベルでカテゴリの説明を保存すると、WYSIWYG 編集者空で表示されていました。
   * _GitHub の問題_: <https://github.com/magento/magento2/issues/38622>
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/38623>
* _AC-11970_:1 つのチェックボックスが選択されたカスタムオプションを使用してコンフィギュレーション可能製品を再注文できない
   * _修正ノート_: システムは、選択されたチェックボックスのカスタムオプションを1つ使用して、選定可能製品の再注文を正しく処理し、バスケットを正常に作成できるようになりました。 以前は、このような商品を並べ替えようとするとエラーが発生し、商品が買い物かごに追加されませんでした。
...
* _AC-12076_:[ 問題 ] 階層型ナビゲーションのフィルター項目の表現を修正
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/37852>
* _AC-12164_:特例文字オプションの 日付 および 時間 形式が機能しない
   * _修正ノート_:システムは、設定された日付形式をタイプ日付の製品カスタムオプションに正しく適用し、日付形式がフロントエンドに正しく表示されるようになりました。 以前は、日付形式構成に対する変更は、タイプ 日付 の製品カスタムオプションのフロントエンドに反映されませんでした。
...
* _ACP2E-3009_: async.operations.all が実行され、エラーが作成されました。
   * _修正ノート_: REST API 呼び出しの製品 リンク データが正しくない場合、重大なエラーが発生しなくなりました。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/a4fbf702>
* _ACP2E-3029_: [クラウド] モバイルの問題のみ PDP 画像をピンチオンできません
...
* _ACP2E-3076_：バンドルオプションを削除中にエラーが発生する
   * _修正ノート_:システムは、エラーをトリガーしたり、ページが応答しなくなったりすることなく、バンドルオプションを正しく削除するようになりました。 以前は、バンドルオプションを削除しようとすると、「ページ応答しない」エラーが発生し、製品が保存されませんでした。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/6a185204>
* _ACP2E-3094_:カテゴリー権限がメモリ不足ブラウザー問題が発生しています。
...
* _ACP2E-3286_：関連製品すべての問題を選択/すべての問題を選択解除
   * _修正注意_:以前は、製品を手動で選択すると、関連製品の「すべてを選択」/「すべてを選択の選択解除」ボタンが正しく機能しませんでした。 修正後は、手動で選択した場合でも、これらのボタンが一貫して機能するようになり、すべての製品が適切に選択または選択解除されるようになります。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/fd5cf3af>
...
* _ACP2E-3350_：無効なカテゴリの名前は、カテゴリツリーでグレー表示されなくなりました
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/d75cff27>
* _ACP2E-3410_:設定可能な製品編集フォームの読み込みにより、タイムアウトとメモリ枯渇が発生する
   * _修正ノート_: 修正前は、すべての可能な属性オプションの組み合わせに基づいて、設定可能な製品バリエーションが構築されていました。 属性に多くのオプションがある場合、これにより、時間とリソースを消費する操作が発生しました。 現在、コンフィギュレーション可能な製品バリエーションは、既存の子製品属性に基づいて構築されます。 これにより、計算が大幅に少なくなり、リソースの使用量が改善されます。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/078c387e>
...
* _ACP2E-3532_:サイトマップ生成の警告
   * _修正ノート_: 修正後、サイトマップはシステムのtmpディレクトリに生成され、最終的な宛先にコピーされます。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/d4de4726>

### コンテンツ

* _AC-10539_:最近表示したウィジェットの価格表示の [問題]  問題
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/38159>
* _AC-10596_: [問題] acl.xsd ファイル内のタイプミスと文法を修正します
...
* _ACP2E-3275_: [Cloud] – 最新の変更がCMS Slider に反映されない
   * _修正ノート_: この問題は、スライドの編集画面で保存イベントがトリガーされている間、スライダーリストが更新されるようにすることで修正されました。 以前は、それが問題を引き起こし、引き起こしていました。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2-page-builder/commit/ae2cdeb0>
* _ACP2E-3326_:ビルダを使用して CMS ブロックを特定の順序で挿入するとページCSM ページでエラーが発生します。
   * _修正ノート_:以前は一部のバージョンのPHPおよびOS(Linux)で、PageBuilderを介して他のcmsブロックを参照するブロックのレンダリングに失敗し、「不明なエラーが発生しました。 もう一度やり直してください。」というエラーメッセージが表示されます。 これで、cms ブロックのコンテンツが、PageBuilder が制御するコンテンツ内で正しくレンダリングされるようになりました。
...
* _ACP2E-3430_：フォントサイズが欠落している TinyMCE 7 を使用した最新のセキュリティ更新
   * _修正注意_:フォントサイズとフォントファミリセレクターが WYSIWYG 編集者で使用できるようになりました。 この修正の前は、TinyMCE 7では編集者インターフェイスでは使用できませんでした。
...
* _ACP2E-2870_:CMS ページ階層によりURL書き換えの問題が発生する可能性があります
   * _修正ノート_:以前は、Webサイト以外のルートページのカスタムパーマネントURL書き換えの場合、無限にリダイレクトされ、ページロードされませんでした。 この修正プログラムを適用すると、Web サイト以外のルート ページのカスタム URL 書き換えは期待どおりに機能し、リダイレクト ループは発生しません。
...
* _AC-10838_:カタログ検索インデックス プロセスのエラー インデックス作成プロセス
   * _修正ノート_:PHPでコンパイルされたlibxmlバージョンに関係なく、システムはエラーを発生させることなくre-indexコマンドを正常に完了するようになりました。 以前は、reindexコマンドを実行すると、PHPが特定のバージョンのlibxmlでコンパイルされたときに、「インデックス作成プロセス中にカタログSearchインデックスプロセスエラー」エラーが発生しました。
   * _GitHub の問題_: <https://github.com/magento/magento2/issues/38254>
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/38553>、 <https://github.com/magento/magento2/commit/0574ac23>
* _AC-10941_:顧客注文クエリにcreated_at、ステータス、grand_totalフィルターが追加され、複数のフィルターの失敗が修正されました。
   * _修正注_: 顧客注文クエリでのcreated_at、ステータス、およびgrand_totalフィルターの使用がサポートされるようになり、複数のフィルターが正しく適用されない問題が解決されました。 以前は、システムはこれらのフィルターをサポートしておらず、クエリで複数のフィルターが使用されると、すべてのを適用できませんでした。
...
* _AC-10991_：関連/アップセル/クロスセルブロックや価格インデックス作成からのクエリがランダムに氾濫する
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/38050>
* _AC-11423_: 例外: 警告: 配列オフセットにアクセスしようとしています... -ICU 74.1 にアップグレードしてから> Calendar.php (PHP Intl)
...
* _AC-8662_: [ 問題 ] cron エラーログの改善
   * _修正ノート_: システムは、cron プロセスの STDERR と STDOUT の両方をキャプチャしてログに記録するようになり、cron プロセスが失敗するシナリオで貴重な診断情報を提供します。 以前は、cron プロセス内のエラーメッセージは記録されず、別のプロセスで実行されている cron グループの STDERR と STDOUT が失われていました。
   * _GitHub の問題_: <https://github.com/magento/magento2/issues/37453>
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/32690>
* _AC-8984_: [問題] 特定のセットアップ CLI コマンドの出力にさらに色を追加します
   * _修正ノート_:システムは、特定のセットアップコマンドラインインターフェイス(CLI)コマンドの出力に色を追加し、読みやすさとユーザーエクスペリエンスを向上させるようになりました。 以前は、これらのコマンドの出力は、色の区別がないために読みづらいものでした。
...
* _AC-9630_:Magentoをアップグレードすると、必要な州/地域を持つ新しい国が追加されたときに、general/region/state_required がリセットされます。
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/38076>
...
* _ACP2E-2692_:部分インデックス作成を実行すると、「ベース テーブルまたは表示が見つかりません」エラーが発生します。
   * _修正ノート_:セカンダリデータベース接続の場合に大きな変更ログで部分的な再インデックスが正しく機能するようになりました
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/ba25af8a>
* _ACP2E-2844_:MariaDB を 10.5.1 以上にアップグレードした後の問題
   * _修正ノート_:MySQLのアップグレード後にDBの日時値が0000-00-00 00:00:00に変換されるときの問題の固定
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/a12063bd>
...
* _AC-7976_: [ 問題 ] GraphQL スキーマにカスタムスカラータイプのサポートが導入されました
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/34651>、 <https://github.com/magento/magento2/commit/0574ac23>
...
* _AC-11729_:ヘッダー値が渡されない場合、Magento_GraphQl はヘッダー処理均等を実行します検証
   * _修正ノート_:システムは、ヘッダー値が検証を渡した場合にのみヘッダー処理が一度だけ実行されるようにし、セキュリティを強化し、潜在的な脆弱性を防ぐようになりました。 従来は、ヘッダー値が検証を通さない均等ヘッダー処理が実行されていたため、ヘッダー値の重複処理による脆弱性や予期せぬ動作が発生していました。
...
* _ACP2E-2809_:複数のパラメーターによる GraphQL 製品のリスト並べ替えが機能しない
   * _修正ノート_:GraphQl の複数のフィールドによる製品の並べ替えが、ドキュメントの説明どおりに機能するようになりました
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/c971859e>
* _ACP2E-948_:商品リスト GraphQL クエリ total_count 10,000 商品のみに制限されています
...
* _AC-12172_:カスタム オプションの種類: ファイルが指定されている場合、製品のインポート時に問題が発生します (作成日 製品にカスタム オプションの価格が含まれておらず、指定された最初のファイル タイプ 拡張子のみが表示されます)
   * _修正ノート_:システムは、「ファイル」タイプのカスタムオプションを使用して製品データを正しくインポートするようになり、指定されたすべてのファイル拡張子が表示され、カスタムオプションの価格が含まれるようになりました。 以前は、製品のインポート中に、タイプ「ファイル」のカスタムオプションに複数のファイル拡張子が指定されている場合、最初の拡張子のみが表示され、カスタムオプションの価格が欠落していました。
...
以前は、は、それぞれのストアではなく管理ストアに割り当てられていました。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/3a7c4d17>
* _ACP2E-2990_:エクスポート時に顧客の「created_at」日付がストアタイムゾーンに変換されない
   * _修正注_:列「created_at」の日付値は、顧客エクスポートCSVセクションのストアタイムゾーンに基づいて適切な日付形式に変換されます。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/3056e9cb>
...
* _ACP2E-3448_: [Cloud] CSV ファイルの誤った数量でエラーが発生しませんでした
   * _修正ノート_:ストックソースのインポートで検証量列の非数値値に対してエラーがスローされるようになりました。 以前は、量列に数値以外の値を持つストックソースをインポートすると、量が 0 に設定されていました。
   * _GitHub コードの投稿_:<https://github.com/magento/inventory/commit/5b21b7af>
* _ACP2E-3455_:URL キーが既にカテゴリに属している場合、商品のインポート時に生成される重複したURLキーエラーメッセージが正しくない
   * _修正ノート_:製品のURLキーがすでにカテゴリに属しているときに顧客製品をインポートしようとすると、製品のインポートチェック中に正しいエラーメッセージが表示されるようになりました。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/d4de4726>
...
* _ACP2E-2102_:管理パネルにワニス 7 ボタン書き出し VCL がない
   * _修正メモ_ 書き出し VCL for Varnish 7」ボタンが管理パネルに追加されました。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/a4fbf702>
...
* _LYNX-395_: ProductInterface の only_x_left_in_stock 属性が、設定可能な製品で正確ではありません
   * _修正メモ_:ProductInterface の only_x_left_in_stock 属性が、買い物かごに設定可能な製品バリアントの使用可能な在庫を正確に反映していない問題を修正しました。 これで、only_x_left_in_stock値は実際のバリアント在庫レベルに正しく対応し、正確な在庫データがカート GQL クエリに返されるようになりました。
...
修正詳細：
* 割り当てられた画像がある場合は、製品のサムネールに正しく表示されるようになりました。
...
* _LYNX-405_: GraphQL エラー：カスタマイズ可能なオプション クエリで&#39;file&#39; タイプがサポートされていません
   * _修正メモ_:GraphQLが、買い物かご項目の「ファイル」タイプのカスタマイズ可能なオプションでエラーを返した問題を修正しました。 クエリは、エラーを発生させずに、ファイルベースのオプションを含む、カスタマイズ可能なすべてのオプションタイプの詳細を正しく返すようになりました。
...
* _LYNX-628_: quantity_return_requested フィールドを追加
   * _修正ノート_: 注文アイテムインターフェイスに 量_return_requested フィールドが追加され、返品が送信されたアイテムの量を識別できるようになりました。 これにより、既存の quantity_returned フィールドと並んで返品追跡が強化されます。
* _LYNX-634_:注文可能なアクションに、すべてのアイテムに対して返品が作成された後に RETURN を含めることはできません 量
...
* _LYNX-643_:setBillingAddressOnCart GQL の same_as_shipping フィールドを使用している場合、アドレス帳に保存が機能しない
   * _修正ノート_:same_as_shippingフィールドを true に設定して setBillingAddressOnCart GraphQL ミューテーションを使用すると、配送先住所が顧客のアドレス帳に保存されない問題が固定。 現在、配送先住所は期待どおりに正しく保存されています。
...
* _LYNX-651_:CustomerOrder で注文コメントが表示されない
   * _修正ノート_: ゲストおよび顧客注文の GraphQL クエリに注文コメントを含めるように CustomerOrder の問題を解決しました。
* _LYNX-652_:original_item_priceに割引を含めることはできません
   * _修正ノート_:GraphQL カート品目価格のoriginal_item_priceロジックを更新し、割引を除外しました。
* _LYNX-681_:バンドル商品の1つが在庫切れの場合、バンドル商品に「IN_STOCK」と表示される
   * _修正ノート_:セット品の1つが在庫切れの場合、セット品のproduct.stock_statusに「IN_STOCK」均等が表示される問題を解決しました。
...
* _AC-10658_:[ 問題 ] visual.phtml のHTML構文エラーを修正しました
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/37457>
* _AC-11474_: [問題] bin/magento メンテナンス:ステータスコマンドで「アクティブ」を「有効」に変更しました。
...
* _AC-12571_:カテゴリツリー内を移動すると、Redis で「Redis セッションが同時接続を超えました」というエラーが発生する
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/0611e750>
* _AC-12731_:dev/css/use_css_重大な__ と組み合わされた CSP の問題パス
   * _修正ノート_: &#39;dev/css/use_css_重大な_パス&#39; 設定が有効になっている場合均等、システムは CSS ファイルをチェックアウトページに非同期的に正しくロードし、これらのページが適切な CSS スタイルでレンダリングされるようになりました。 以前は、制限付きコンテンツセキュリティポリシー(CSP)によりインラインJavaScriptの実行が妨げられ、CSSファイルが期待どおりに読み込まれませんでした。
   * _GitHub の問題_: <https://github.com/magento/magento2/issues/39020>
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/39040>
* _AC-13398_:仮想型を使用してプラグインを設定し、setup:di:compile コマンドでインターセプターメソッドを正しく生成できない
   * _修正ノート_: 仮想型を使用してプラグインを構成するときに、システムがインターセプター メソッドを正しく生成するようになり、プリコンパイル済みかランタイム コンパイル済みかに関係なく一貫した結果が得られるようになりました。 以前は、ランタイムのコンパイルと比較して事前コンパイルすると、システムが誤った結果を生成していました。
...
* _ACP2E-3631_:Adobe Systemsコマース 2.4.7-p3 単体テストが失敗する
   * _修正注意_:リリースノートは必要ありません。
...
* _ACP2E-3377_:バンドル製品を追加すると、買い物かごルールの「買い物かご全体の固定金額割引」アクションが割引を誤って適用します
   * _修正点_：固定金額の買い物かごルールがバンドル製品に正しく適用されていませんでした。 これで、合計割引額を計算するときに、バンドルの子製品が検討され、適切な割引計算が行われます。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/1366ae5e>
...
* _ACP2E-3491_：請求書で買い物かごルールの SKU 条件が失敗する。
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/3f12d152>
* _ACP2E-3498_：複数の買い物かご価格ルールを割引/特別価格の製品と同時に適用した場合の、誤った割引値
   * _修正ノート_: 修正前は、複数のルールが適用されている場合、買い物かごルール全体の固定量が正しく適用されていませんでした。 現在、定額割引買い物かごルールが適切に適用されています。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/1984c61c>

### 収益

* _ACP2E-3330_: [クラウド] 制限付き管理者ユーザーが戻るメニューとボタンを表示できる
   * _修正ノート_:制限付き管理者ユーザーは、RMA関連のコントロール(メニューとボタン)にアクセスできないようになりました。以前は管理者ユーザーによって制限されていたが、「戻る」メニューおよびボタンを表示できた問題を修正しました。
* _ACP2E-3443_：画面を更新すると、戻り画面が表示されません
   * _メモの修正_：ユーザーは、画面のひずみを経験せずにページを更新できます。

### SEO

* _AC-11907_:アクセント付きのURL書き換えを追加すると、読み込みが無限になります
   * _修正ノート_:システムは、アクセント付きの書き換えURL正常に作成および機能するようになり、保存プロセス中に無限の読み込みが防止されるようになりました。 以前は、アクセント付きのURL書き直しを追加すると、読み込みが無限になるという問題がありました。
...
* _AC-11938_:UPS REST 「出荷単位として KGS/IN、LBS/CM、または OZS/CM を使用することはできません。」
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/493e01f5>
...
* _AC-11654_:JSON 列タイプが原因で統合テストが testDbSchemaUpToDate に失敗する
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/ef81f5a2>
* _AC-13362_: [問題] PHPDoc 修正スペル
   * _修正ノート_: PHPDoc のスペル修正により、システムは IDE で非推奨のメソッドを正しく認識するようになりました。 以前は、PHPDoc のスペルミスにより、IDE は特定のメソッドを非推奨として認識しませんでした。

...
* _BUNDLE-3372_:クレジットカード - ティアダウン機能
   * _修正点_：お客様が支払いページから配送ページに戻った場合、システムはBraintree PayPal コンポーネントのティアダウンを正しく処理するようになりました。これにより、エラーが防止され、PayPal Express ボタンが正しくレンダリングされます。 以前は、支払ページから出荷ページに戻ると、Braintree PayPalコンポーネントを破棄しようとするとエラーが発生することがありました。
...
* _AC-11641_: [ 問題 ] [ チェックアウト ] 失敗した支払いメールテンプレートで更新された依存ディレクティブ
   * _修正ノート_:システムは、仮想製品の失敗した支払い電子メールテンプレートから出荷先住所と配送方法を正しく省略し、関連情報のみが電子メールに含まれるようにするようになりました。 以前は、仮想製品の失敗した支払いメールに配送先住所と配送方法が誤って含まれていました。
   * _GitHub の問題_: <https://github.com/magento/magento2/issues/32781>
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/32511>
* _AC-11717_:Firefox ブラウザー でコンソールエラーが発生する既存顧客チェックアウト内の Magento 2 ログイン
   * _修正注意_: Firefox ブラウザー でコンソールエラーが発生することなく、チェックアウトプロセス中にユーザーがログインできるようになりました。 以前は、チェックアウト 中に既存顧客としてログインしようとすると、Firefox でコンソールエラーが発生していました。
   * _GitHub の問題_: <https://github.com/magento/magento2/issues/38557>
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/39509>
* _AC-11876_: [問題] 2.4.7 での売上ルールのリグレッション
   * _修正ノート_: システムは販売ルールを正しく検証するようになり、製品コンディションがどの製品名とも一致しない場合にクーポンコードが買い物かごにアプリケーションされるのを防ぎます。 以前は、商品コンディションがどの商品名とも一致しない場合、販売ルールが適用され、送料均等割引を受けることができました。
...
* _AC-11993_: [問題] ローダーは、郵便番号、配送料、ルールが変更された後、配送方法をブロック検証
   * _修正ノート_:システムは、配送料検証ルールなしでカスタム配送方法を正しく処理するようになり、チェックアウト中に配送先住所の郵便番号が変更された後にローダーが配送方法をブロックしないようにしました。 以前は、チェックアウト中に配送先住所の郵便番号を変更すると、配送料検証ルールのないカスタム配送方法が使用された場合、ローダーは配送方法をブロックし、消えませんでした。
...
* _AC-13324_: bin/magento sampledata:deploy —no-update が例外をスロー
   * _修正点_:sampledata:deploy コマンドで – no-update オプションを使用する場合、システムがブール値を正しく受け入れるようになり、サンプルデータのデプロイ中にエラーが発生しなくなりました。 以前は、このコマンドを使用すると、システムが整数値を誤って予期していたため、エラーがスローされました。
   * _GitHub の問題_: <https://github.com/magento/magento2/issues/39344>
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/39345>
* _AC-13355_: [問題] EAV キャッシュタイプの使用法を修正
...
* _ACP2E-2905_:マルチウェブサイトアーキテクチャにおける [クラウド] 見積もりの問題
   * _修正注意_:以前は、異なる通貨と顧客グループを持つ複数のWebサイトアーキテクチャでは、ストアに割引を正しく適用できませんでした。 この修正が実装されると、顧客グループ価格の割引が異なる複数の web サイトアーキテクチャが様々なストアに正常に適用されます。
...
* _ACP2E-2909_: dynamic-rows.js:658 Uncaught TypeError: バンドル製品の編集中に dataRecord.slice がキャッチされました
   * _修正注意_:バンドル製品からオプションを削除する際にブラウザーコンソールに JavaScript エラーはありません。
   * _GitHub の問題_: <https://github.com/magento/magento2/issues/38505>
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/93d50f8d>
* _ACP2E-2950_: [クラウド] バンドル製品の注文確認の価格が間違っている
   * _修正ノート_:基本通貨以外の通貨が使用された場合、ストアフロントでバンドルオプションの正しい金額が表示されます。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/a4fbf702>
...
* _ACP2E-3100_: [Cloud] 画像ファイルがNew Relic エラーログに存在しません
   * _修正ノート_:システムはカスタムプレースホルダー画像をローカルストレージに同期するようになり、AWS S3などのリモートストレージを使用するときに正しくレンダリングされるようになりました。 以前は、リモートストレージを使用する際に、カスタムプレースホルダー画像のレンダリングに失敗し、画像表示とエラーログが壊れていました。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/d1f7dc95>
...
* _ACP2E-3126_: [Cloud] Product Media Gallery GQL 応答が画像の位置で並べ替えられていません
   * _修正点_：メディアギャラリー内の項目がGraphQL レスポンス内の位置で正しく並べ替えられ、正確な表示順が確保されるようになりました。 以前は、メディア ギャラリー内のアイテムは位置で並べ替えられていなかったため、表示順序が正しくありませんでした。
   * _GitHub の問題_: <https://github.com/magento/magento2/issues/37671>
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/b21e5d91>
* _ACP2E-3136_: [クラウド] 管理バックエンドのウィジェット編集にサブカテゴリー項目が表示されません
...
* _ACP2E-3131_: コンテンツウィジェットで [ クラウド ] アンカー/アンカー以外のカテゴリが反転されました
   ...
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/7377de59>
* _ACP2E-3152_:カテゴリがウィジェットで機能しません。
   * _修正ノート_:以前は、さまざまなアンカー/非アンカーカテゴリのCMSブロックを保存した場合、フロントエンドに表示されたときに子カテゴリでは機能しませんでした。 この修正が適用されると、ブロックが様々なカテゴリのフロントエンドに表示されます。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/d01ee51e>
...
* _ACP2E-2672_: [クラウド] 多数の製品を同時に更新すると、特別価格 API エンドポイントがエラーを返す
...
* _ACP2E-3053_：特定のカテゴリページでの [Cloud] Elastic search error
   ...
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/c8931218>
* _ACP2E-3345_:Magento\CatalogSearch\Model\Indexer\Fulltext\Interceptor のオブジェクトの作成時に書式 ＜例外＞Photoshop のみ「テキスト」エラーが発生しました。
   * _修正ノート_: 修正後、$dataを指定せずに Magento\CatalogSearch\Model\Indexer\Fulltext クラスのインスタンスを作成できます。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/1366ae5e>
...
* _ACP2E-3195_：ギフトレジストリ項目の注文中に配送先住所が空になる
   * _修正注意_:以前は、ゲストユーザーギフトレジストリアイテムの場合、電子メール機能から返されると、空の空白のアドレスが生成されましたが、これは注文を行うのに正しくありません。 この修正が適用されると、ギフトレジストリはログインしているユーザー/ゲストユーザーと割り当てられたアドレスが存在する場合をチェックします。

### 雲

* _ACP2E-3010_: [クラウド] PHPSESSID が各POSTリクエストを変更しています
   * _修正ノート_:L2 Redisキャッシュが有効になっていて、顧客がバックエンドから更新された場合、ログイン顧客のフロントエンド領域のPOSTリクエストでPHPSESSIDが再生成されなくなりました
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/6a185204>
...
* _AC-10539_:[ 問題 ] 最近表示された項目ウィジェットの価格表示の問題
   * _修正ノート_:システムは「最近表示された製品」ウィジェットに在庫切れのシンプルな製品の価格を正しく表示し、すべてのウィジェットと製品リストページで一貫性を確保するようになりました。 以前は、価格読み込みテンプレートの条件により、「最近表示された商品」ウィジェットに在庫切れのシンプルな商品の価格が表示されませんでした。
...
* _AC-12283_:2.4.5-p8 への更新後、「市外局番が設定されていません」
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/38922>
* _AC-12692_:Widgetカテゴリツリーが正しくレンダリングされません
   * _GitHub の問題_: <https://github.com/magento/magento2/issues/39008>
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/58e40ceb>
* _AC-13054_:デザイン設定ページのテーマを変更すると、「デフォルト値を使用中」というメッセージが表示されません。
   * _メモの修正_：デザイン設定ページで選択したテーマに応じて、「デフォルト値を使用」メッセージを表示する別の列がシステムに含まれるようになりました。 これにより、デフォルト値のステータスが明確かつ可視化されます。 以前は、「デフォルト値を使用しています」というメッセージが表示されなかったため、選択したテーマのステータスについて混乱が生じていました。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/47b448e2>
...
* _ACP2E-2606_：ページビルダーでYouTube nocookie の URL が機能しない
   * _注意_:ページビルダーは、検証ルールのフォーム要素設定でYouTubeのcookie禁止URLを許可するようになりました。 以前は、YouTube のcookie禁止 URL が pagebuilder で機能していませんでした。
* _ACP2E-2693_:ニュースレターテンプレートの問題により [クラウド] フロントエンドが読み込まれない
   * _修正ノート_:内容セクションページCMS経由でブロックを追加しても例外リードしなくなりました
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/ea79f7dd>
...
* _AC-8499_:国のドロップダウンが変更された場合、地域のテキストフィールドがリセットされません
   * _修正注意_:ドロップダウンメニューで国が変更されたときに、システムが地域テキストフィールドをリセットし、以前の値が保持されないようにするようになりました。 以前は、ドロップダウンリストから国を変更しても地域フィールドがリセットされず、最後に保存された値が保持されていました。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/3ea26621>
* _AC-9240_:顧客を削除してもすべてを選択ログインして削除された顧客のストアフロントにあるブラウザーセッションデータがクリーンアップされない
   * _修正ノート_:顧客を削除すると、ログインしている顧客と削除された顧客のストアフロントからすべてのブラウザーセッションデータが期待どおりにクリーンアップされるようになりました。 買い物客は買い物を続けることができ、ブラウザーはそのセッションをゲストセッションとして扱います。 以前は、ログインしている買い物客の顧客アカウントが管理者から削除されると、買い物客のブラウザーによってJavaScriptエラーが発生していました。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/7d5e3906>

### フレームワーク

* _AC-10037_: [質問]未使用の 書式 ＜例外＞Photoshop のみ「テキスト」 の設定 `app/code/Magento/Translation/etc/di.xml`
...
* _AC-10654_:V1/顧客/パスワード エンドポイントの質問/問題
...
* _AC-12869_:[ 問題 ] Magento モジュールで参照されている誤ったクラスを修正しました。
   * _修正ノート_:システムはモジュール内のクラスを正しく参照するようになり、よりスムーズな操作が保証され、存在しないクラスによるクラッシュが防止されます。 これには、Indexer モジュールと Creditmemo モジュールのバグ修正、および PrintAction クラスの HttpGetActionInterface の実装が含まれます。 以前は、誤ったクラス参照によりエラーや潜在的なシステムクラッシュが発生し、クレジットメモPDFファイルのファイル名や株式のインデックス再作成などの特定の機能が期待どおりに機能していませんでした。
...
* _AC-12964_:dev:di:info CLI コマンドの領域を定義する機能
   * _修正点_：このシステムでは、開発者が dev:di:info CLI コマンドの領域を定義できるようになり、開発とデバッグプロセスが強化されました。 以前は、このコマンドは GLOBAL 領域の情報しか表示できませんでした。
   * _GitHub の問題_: <https://github.com/magento/magento2/issues/38758>
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/38759>
* _AC-13149_: [問題] isMultipleFiles プロパティ を画像フォーム要素に追加しますテンプレート
   * _修正ノート_:この修正により、複数ファイルの画像フォーム要素に画像が追加されたときに、「ここに画像を見つけるかドラッグする参照」ボタンが消えなくなります。
...
* _AC-13810_：スケジュールで更新モードで、カスタマーグリッドインデクサーが正しく機能しない
   * _修正注_:以前の顧客グリッドは即座に更新されましたが、修正後顧客グリッドはcronの実行後に更新されますが、すぐには反映されません。
...
* _AC-8353_: [問題] 削除 `@author` 禁止されているタグ
   * _修正ノート_:システムは、特定のモジュールから禁止されている `@author` タグを削除することでコーディング標準に準拠するようになり、よりクリーンで標準化されたコードが保証されます。 以前は、 `@author` タグが一部のモジュールに存在していましたが、これは確立されたコーディング標準に反していました。
...
* _ACP2E-3375_: [CLOUD] 管理者のコンソールには、多くの JavaScript エラーがあります
   * _修正注意_:以前は、管理パネルでコンソールに多くのJavaScriptエラーがあります。 これで、管理パネルでは、コンソールにJavaScriptエラーが表示されなくなり、すべてのデフォルトのJavaScript機能が問題なく正常に実行されます。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/d75cff27>
* _ACP2E-3387_: [Cloud] Magento: キューメッセージが削除されました
   * _修正ノート_:キューメッセージが適切にクリアされるようになりました。 修正の前は、SQL キューシステムが使用されていたので、クリーニング キューメッセージが同時に実行されている場合は、新しいメッセージが削除される可能性がありました。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/d50f6b5d>
...
* _ACP2E-2642_：コンテンツのステージングの更新で、Fastly キャッシュがクリアされない
   * _注意_: PageBuilder 内容関連エンティティが更新されると、PageBuilder コンテンツを含む GraphQL 応答キャッシュが無効になるようになりました。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/ba25af8a>
* _ACP2E-2653_:レイヤードナビゲーションの無効化 - Graphql から集計を削除しない
...
* _ACP2E-3128_:[Cloud] ノードの見積もりを使用した getPurchaseOrder のGraphQL呼び出しの不具合
   * _修正ノート_:購入注文GraphQL呼び出しは、内部サーバーエラーが発生することなくタスクを実行できます。
...
* _ACP2E-3184_:「すべてのストア表示」で製品が有効になっていない場合、[Cloud] 設定可能な製品が実稼動サイトに表示されない
   * _修正ノート_:製品が「すべてを選択ストアビュー」で有効になっていないが、特定のストア表示スコープで有効になっている場合、システムはサイト均等に構成可能な製品を正しく表示するようになりました。以前は、商品が「すべてのストア表示」で無効になっていて、特定のストア表示範囲でのみ有効になっている場合、GraphQLの応答で商品の属性が正しく表示されず、商品が正しく表示されませんでした。
...
以前はクエリマルチサイトセットアップで顧客がデフォルト以外のWebサイトに存在することを確認しずに機能していました。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/581b7ef1>
* _ACP2E-3253_:GraphQL 買い物かご itemsV2 ページネーションが正しく機能していません
   * _修正注意_: この問題は、コレクション クエリで現在の ページ 引数の正しい値を渡すことで修正されました。 以前は、現在のページを設定するために間違った値が渡され、この問題を引き起こしていました。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/8459b17d>
...
* _ACP2E-3492_:Graphql API に関する [クラウド] の問題
   * _修正ノート_:Graphqlアプリケーションサーバーを使用して修正する前は、アドレスリクエスト顧客最新のデータが返されませんでした。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/3f12d152>
* _ACP2E-3505_:無効商品が grpahQL の関連、アップセル、クロスセル商品にまだ表示されますクエリ
   * _修正ノート_:Graphqlは、無効な関連製品、アップセル製品、およびクロスセル製品に対して正しい応答を提供するようになりました
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/d4de4726>
...
* _LYNX-510_:OrderAddress でselected_optionsをクエリするときにエラー
   * _注意_: 注文アドレスの GraphQL 応答でcustom_attributesV2ように AttributeSelectedOptions を更新しました。
* _LYNX-512_:original_item_priceには割引が含まれていません
   * _修正ノート_:割引を含めるようにoriginal_item_priceを更新しました。
...
* _LYNX-581_:cookie関連プロパティを非公開から保護に移動
...
* _ACP2E-3382_：書き出された顧客住所はインポートできません
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/ec7e32a9>
* _ACP2E-3448_: [クラウド] ファイルの間違った量CSVエラーが発生してもエラーは発生しませんでした
   * _メモの修正_：在庫ソースの読み込みが行われると、数量列の数値以外の値に対して検証エラーがスローされるようになりました。 以前は、「数量」列に数値でない在庫ソースを読み込むと、数量が 0 に設定されていました。
   * _GitHub コードの貢献_: <https://github.com/magento/inventory/commit/5b21b7af>
...
* _ACP2E-3475_:製品のエクスポートにより、4G メモリ制限で OOM 均等が発生する
   * _修正ノート_:この修正前、製品属性に数千のオプション値があり、4G の使用可能なメモリがある場合、製品のエクスポート均等失敗しました。 この修正後、製品のエクスポートで csv ファイルのエクスポート終了必要があります。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/1984c61c>
...
* _ACP2E-3476_: [クラウド] 製品のインポート時間が大幅に増加しました
...
* _AC-10750_:データベースでプレフィックスを使用すると、選定可能製品の在庫更新が失敗する
...
* _ACP2E-2734_：メールの送信に失敗する
   ...
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/b2286ecf>
* _ACP2E-2756_: [Cloud] 部分的に発送された注文の一部返金時に注文ステータス完了するように変更されます
   * _修正ノート_:クレジットメモを発行するときに、まだ出荷されていないアイテムがある場合、注文ステータスが「完了」に変更されなくなりました。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/7e0e5582>
...
* _LYNX-699_:placeOrder GQL で null 非許容フィールド「TaxItem.title」に null を返せない
   * _注意_: null 非許容フィールド TaxItem.title に null 値があるために、placeOrder ミューテーションが内部サーバー エラーで失敗した問題が固定。 これで、フィールドは常に有効な値を返し、注文配置が正常に完了するようになりました。
* _LYNX-702_: EstimateTotals：仮想商品タイプのディスカウントが null です
   * _修正ノート_:仮想製品を含む買い物かごに割引コードが適用された場合に、estimateTotals変異が割引に対してnullを返す問題を解決しました。
* _LYNX-703_:バンドル商品が正しい割引率と金額を返さない
...
* _LYNX-714_：製品レベルでのギフトメッセージ設定
   * _修正ノート_:グローバルに無効になっている場合に、ギフトメッセージが商品レベルで適用されなかった問題を固定しました。 これで、特定の製品に対してギフトメッセージが有効になっている場合、updateCartItems ミューテーションを使用してギフトメッセージを正常に追加でき、正しく保存および反映されます。
* _LYNX-717_:買い物かごアイテムからギフト用ラッピングを削除する際の問題
   * _修正注意_:updateCartItems ミューテーションを使用して買い物かごアイテムからギフト用ラッピングを削除すると、期待どおりに機能しなかった問題を固定しました。 これで、ギフトラッピングの適用と削除の両方がエラーなしで正しく機能するようになりました。
...
* _AC-11932_:デフォルト製品属性設定の問題
   * _メモの修正_：製品属性のデフォルトオプションの選択を解除できるようになり、属性に必ずしもデフォルトが設定されるわけではありません。 以前は、製品属性にデフォルトが設定されると、その選択を解除する方法がなく、属性には常にデフォルトが設定されていました。
...
* _ACP2E-2602_：招待からアカウントを作成する際に、顧客属性が表示されない
   * _修正注意_:招待からアカウントを作成するときに顧客属性を使用できるようになりました。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/39d54c2d>
* _ACP2E-2627_：注文のキャンセルで失敗した支払で、クーポンごとの使用制限を持つクーポンコードがリリースされない
   ...
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/c971859e>
* _ACP2E-2811_: [クラウド] カタログ ルール製品のインデックス再作成インデクサーで SQLSTATE[HY000]: 一般 エラー: 2006 MySQL サーバーが削除されました。
   * _修正ノート_:システムは、「Magento\CatalogRule\Model\Indexer\IndexBuilder」のdi.xml内のカスタム「batchCount」値を正しく処理するようになり、大規模なカタログでのバッチサイズが正しくないため、カタログルール製品インデクサーのインデックス再作成中に「一般エラー:2006 MySQL サーバーがなくなりました」などの SQL エラーが防止されるようになりました
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/b2286ecf>
* _ACP2E-2926_:訪問者に対する [クラウド]カートの価格ルール 顧客が買い物かごに割引を適用していないセグメント
   * _修正ノート_:ルールがクーポンを使用しない場合均等、訪問者の顧客セグメントにカート価格ルールが正しく適用されるようになり、適切な割引が買い物かごに適用されるようになりました。 以前は、買い物かごの価格ルールでクーポンを使用しない限り、訪問者の顧客セグメントの買い物かごに割引が適用されませんでした。
...
* _ACP2E-3406_：ルール条件内のネストされたカテゴリが表示されない
   * _修正ノート_:レベル 3 カテゴリ 未満のネストされたカテゴリがカテゴリ条件のマーケティングルールに表示されない場合の問題固定
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/88660e79>
...
* _ACP2E-3456_：買い物かごの価格ルールで「次と等しいか、次よりも大きい」条件を使用している場合、親カテゴリが考慮されません。
   * _修正ノート_:カートの価格ルール高度な条件で使用される場合に、親カテゴリを正しく考慮するようになりました
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/93359343>
* _ACP2E-3463_:無効な割引計算が優先されます
   * _修正注_: 買い物かご割引タイプ全体に適用される固定金額の場合、前のプロモーションによって既に割引された買い物かご品目の金額が正しく計算されていませんでした。 今では、割引は正しく合計されます。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/078c387e>
...
* _ACP2E-3273_:ReCaptcha V2 がドイツ語言語のチェックアウトで正しく表示されません。
   * _メモの修正_：以前は、チェックアウトからのメールアドレスの下にある recaptcha は、ドイツ語などの長い単語を含む言語ではスタイルが設定されていないように見えます。 この後、reCAPTCHA は残りの領域のすべての reCAPTCHA 要素と同じように見えます。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/7377de59>
* _ACP2E-3300_:管理ログインのCaptchaは、一部のユーザーの操作を必要としません
...
* _AC-9432_: [ 問題 ] メンテナンス許可リストで CIDR 範囲を使用できるようにします
   * _修正ノート_: メンテナンスモードでの CIDR 範囲の使用がサポートされるようになりました。IP リストを許可し、IP アドレスの範囲でメンテナンスモードをバイパスできるようになりました。 以前は、メンテナンス モードでは、IP リストは 個人 IP アドレスのみがメンテナンス モードをバイパスできました。
...
* _ACP2E-3193_:固定製品税(FPT)が選定可能製品で機能しない
   * _修正ノート_:設定可能な製品バリエーションのFPTが正しく機能しています。
   * _GitHub コードの投稿_:<https://github.com/magento/magento2/commit/ec7e32a9>
...
* _AC-12128_:プロトタイプ.js セキュリティ脆弱性の修正 CVE-2020-27511
   * _修正ノート_:プロトタイプ.js 1.7.3のセキュリティ脆弱性CVE-2020-27511に対処するためにシステムが更新され、システムの全体的なセキュリティが強化されました。 この更新の前は、細工されたHTMLタグを削除することにより、システムは標準式サービス拒否(ReDOS)の影響を受けやすくなっていました。
...
* _AC-12650_:[ 問題 ] 開発者モードで、より少ないファイル数で sourceMap の生成を追加
   ...
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/pull/38977>
* _AC-1306_:無効なモジュールに対して 静的 内容 がデプロイされています
...
* _AC-13459_：最小在庫しきい値を使用した「在庫切れ」の並べ替えで一貫性のない動作が発生する
   * _メモの修正_：システムでは、設定された最小在庫しきい値に準拠し、在庫切れの項目を一貫してリストの下部に移動し、在庫レベルに基づいてカタログ内の製品を正しく並べ替えるようになりました。 以前は、並べ替え動作に一貫性がなく、アイテムが在庫レベルに基づいて常に正しい順序で表示されるとは限らず、カテゴリ階層を保存、更新、または変更した後に並べ替えの変更が予期せず発生する可能性がありました。
   * _GitHub コードの貢献_: <https://github.com/magento/magento2/commit/47b448e2>

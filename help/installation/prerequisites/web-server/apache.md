...
1. 分散`.htaccess`構成ファイルを使用するようにアプリケーションを有効にします。
   ...
   >[!NOTE]
   >
   >以前のバージョンのApacheからアップグレードした場合は、まず`000-default.conf`で既存の`<Directory "/var/www/html">`または`<Directory "/var/www">` ブロックを探します。 別の`docroot`にAdobe Commerceをインストールする場合は、そのパスに一致する`<Directory>` ブロックを更新します。
...
1. Ubuntu](#enable-rewrites-and-htaccess-for-ubuntu)の[書き換えと`.htaccess`を有効にします。

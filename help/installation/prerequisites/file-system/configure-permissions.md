...
使用する手順は、 [共有ホスティング](#set-permissions-for-one-user-on-shared-hosting) または [プライベートサーバー](#set-ownership-and-permissions-for-two-users) とには 2 人のユーザーがいます。
...
1. コマンドラインアクセス権がある場合は、次のコマンドを次の順に入力します。
   ...
   1 行にすべてのコマンドを任意で入力するには、次のように入力します ( アプリケーションが `/var/www/html/magento2`:
...
このセクションでは、ファイル・システムの所有者を作成し、そのユーザーを Web サーバのグループに配置する方法について説明します。 必要に応じて、既存のユーザーアカウントを使用できます。セキュリティ上の理由から、ユーザーには堅牢なパスワードを使用することをお勧めします。
...
このセクションでは、ファイルシステムの所有者を作成する方法について説明します。 ( ファイルシステム所有者は、 *コマンドラインユーザ*.)
...
>[!NOTE]
>
>この `-a -G` オプションは、 `apache` または `www-data` as a *セカンダリ* ユーザーの *プライマリ* グループ化します。 セカンダリグループをユーザーアカウントに追加すると、 [ファイルの所有権と権限を制限](#set-ownership-and-permissions-for-two-users) 共有グループのメンバーが特定のファイルにのみアクセスできるようにする。
...
1 行にすべてのコマンドを任意で入力するには、次のように入力します ( アプリケーションが `/var/www/html/magento2` Web サーバーのグループ名は `apache`:

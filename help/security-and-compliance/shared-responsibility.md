...
<tbody>
  ...
  <tr>
    <td>アクティブにサポートされているバージョン（インフラストラクチャサービス、SaaS Commerceアドオン、サードパーティの拡張機能を含むがこれに限定されない）に基づくすべてのプラットフォームサービス、サードパーティの依存関係、およびCommerce サービス拡張機能の管理</td>
    <td>I</td>
    <td>R</td>
    <td></td>
    <td></td>
...
<tfoot>
  <tr>
    <td colspan="5">
      <p><sup>1</sup> Adobe Commerce オンクラウド インフラストラクチャ リポジトリがメイン リポジトリとして使用されている場合のみ。 その他の外部リポジトリの使用は、販売者の唯一の責任です。</p>
      <p><sup>2</sup> Adobeは、CDN プロバイダーの問題に対してレベル 1のサポートを提供します。</p>
      <p><sup>3</sup> マーチャントは、アプリケーション用に設定したNginx コントロールを担当します。</p>
      <p><sup>4</sup> PCIの場合、Adobeと販売店の間で侵入テストの要件が共有されます。</p>
    </td>
  </tr>
</tfoot>
</table>
...
| マーチャントは、サポートされているバージョンでPHPを管理する必要があります。 サポートされていないバージョンでのデプロイメントは、Adobe サポートの対象外であり、パッチが適用されていないセキュリティの脆弱性が含まれている可能性があります。 |     | R |
...
| 既定のデータベース設定の継続的なメンテナンス <br><br> （インデックス作成とコア テーブルの最適化、既定のシステム管理設定の最適化） | R |   |
...
| マーチャントは、サポートされているバージョンでMariaDBを管理する必要があります。 サポートされていないバージョンでのデプロイメントは、Adobe サポートの対象外であり、パッチが適用されていないセキュリティの脆弱性が含まれている可能性があります。 |     | R |
...
| --- | --- | --- |
| CDNの可用性と品質 | R |   |
| Fastly サービス設定（拡張機能またはAPI経由） |     | R |
| Fastlyの拡張品質 | R |   |
| Fastly統合VCL スニペットの品質（Fastly拡張機能にバンドル） | R |   |
| ページキャッシュの最適化 |     | R |
...
| マーチャントは、サポートされているバージョンでRedisを管理する必要があります。 サポートされていないバージョンでのデプロイメントは、Adobe サポートの対象外であり、パッチが適用されていないセキュリティの脆弱性が含まれている可能性があります。 |     | R |
...
| --- | --- | --- |
| OpenSearchの可用性 | R |   |
| デフォルトのOpenSearch設定の設定 | R |   |
| インストール済みのAdobe Commerce バージョンと互換性のあるOpenSearch バージョンをインストールするには、サービスリクエストを送信します |   | R |
| マーチャントは、サポートされているバージョンでOpenSearchを維持する必要があります。 サポートされていないバージョンでのデプロイメントは、_not_ Adobe サポートの対象であり、パッチが適用されていないセキュリティ上の脆弱性が含まれている可能性があります。 |     | R |
...
| 送信トランザクションメールにのみサービスを使用します。 このサービスは、マーケティングメールの送信をサポートしていません。 |     | R |
...
|     | Adobe | 加盟店 |
| --- | --- | --- |
| Advanced Reporting サービスの可用性 | R |   |
| 詳細レポートの設定は、詳細レポートの利用条件に準拠しています |     | R |
...
| 最新リリース版の製品レコメンデーションのみがサポートされています。 販売者は、リリースごとに迅速にアップグレードする必要があります。 |     | R |
...
| ライブサーチの最新リリース版のみがサポートされています。 販売者は、リリースごとに迅速にアップグレードする必要があります。 |     | R |
...
| PWAの導入 | R |   |
| PWAのカスタム実装 |  | R |
| Edge Delivery Servicesの導入（Commerceボイラープレート） | R |   |
| Edge Delivery Servicesのカスタム実装 |   | R |
...
| 決済サービスの可用性 | R |   |
| 支払いモジュールのアップグレード |   | R |
| 最新のリリース済みバージョンの決済サービスのみがサポートされています。 販売者は、リリースごとに迅速にアップグレードする必要があります。 |     | R |
...
| 専用SSL証明書の有効期限 | R |   |
| SSL証明書のプロビジョニング | R |   |
| EVまたはその他の特定のSSL証明書（提供されるデフォルト以外）を購入および管理し、Adobeに提供する |     | R |
...
| マーチャントのVPC サービスエンドポイントへのPrivateLink インバウンドの設定 |     | R |
...
|     | Adobe | 加盟店 |
| --- | --- | --- |
| CPU リソース、データセンター、およびディスク領域の可用性 | R |   |

...
サードパーティ製ツール、HTTP/2、非推奨のJSおよびCSS結合に関するガイダンスについては、*設定のベストプラクティス*&#x200B;の[ バンドルヒント ](configuration.md#bundling-tips)を参照してください。
...
この手順では、ストアの`requirejs-config.js` ファイルの複数の`deps`、`shim`、`paths`、および`map`設定ノードをすべて、`build.js` ファイルの対応するノードに集約する必要があります。 これを行うには、ブラウザーの開発者ツールパネルで「**[!UICONTROL Network]**」タブを開き、ホームページなど、ストア内の任意のページに移動します。 「ネットワーク」タブで、上部の近くにストアの`requirejs-config.js` ファイルのインスタンスが表示され、ここで強調表示されます。

![[!DNL RequireJS]設定](../assets/performance/images/RequireJSConfig.png)

このファイル内には、各設定ノード （`deps`、`shim`、`paths`、`map`）に対する複数のエントリがあります。 複数のノード値をbuild.js ファイルの単一の設定ノードに集約する必要があります。 例えば、ストアの`requirejs-config.js` インスタンスに15個の個別の`map` ノードのエントリがある場合、15個のすべてのノードのエントリを`build.js` ファイルの単一の`map` ノードにマージする必要があります。 `deps`、`shim`、`paths`のノードについても同じです。 このプロセスを自動化するスクリプトがなければ、時間がかかることがあります。

次のように、`paths`設定ノードのパス `mage/requirejs/text`を`requirejs/text`に変更する必要があります。
...
- `name`— バンドルの名前。 例えば、`bundles/cart`という名前は、`bundles` サブディレクトリに`cart.js` バンドルを生成します。
...
#### &#x200B;4. バンドルを使用するように[!DNL RequireJS]を設定

バンドルを使用するように[!DNL RequireJS]を取得するには、`build.js` ファイルの`modules` ノードの後に`onModuleBundleComplete` コールバックを追加します。
...
`pub/static/frontend/Magento/luma/en_US` ディレクトリで`requirejs-config.js`を開き、[!DNL RequireJS]が構成呼び出しをバンドルしてファイルを追加したことを確認します。
...
`build.js` ファイルでオプティマイザーを有効にするには、`build.js` ファイルの先頭にあるoptimize プロパティの値として`uglify2`を追加します。

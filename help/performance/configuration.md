...
グリッド内の製品の合計数が制限されている場合、製品グリッドのマスアクションには影響しません。 商品グリッドのプレゼンテーション層にのみ影響します。 例えば、グリッド内の製品の数は限られており、**[!UICONTROL Select All]**&#x200B;をクリックし、**[!UICONTROL Update attributes]**&#x200B;個のマス アクションを選択し、一部の属性を更新します。 `20000`その結果、すべての製品が更新され、`20000`件のレコードの限定コレクションは更新されません。

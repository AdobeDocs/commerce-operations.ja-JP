...
description: Adobe Commerceの graphql アプリケーションサーバーについて説明します。 実装に関するガイダンスと最適化戦略を確認します。
...
## クライアント IP 検出用の alternativeHeaders の設定

デフォルトの場合、GraphQL Application Server は、`x-forwarded-for` ファイルで定義されている `app/etc/di.xml` ヘッダーの標準設定をサポートし、一般的なプロキシおよび CDN 環境のクライアント IP アドレスを正確に取得できます。

追加またはカスタムヘッダー（`x-client-ip`、`fastly-client-ip`、`x-real-ip` など）をサポートする必要がある場合は、`alternativeHeaders` ファイルで `app/etc/di.xml` 引数を拡張または上書きできます。 これは、環境で `x-forwarded-for` 以外のヘッダーを使用してクライアント IP アドレスを渡す場合にのみ必要です。
...
必要に応じてヘッダーを追加、削除または並べ替えて、クライアント IP が設定に適したソースから取得されるようにすることができます。

>[!NOTE]
>
>Fastly CDN モジュールで Adobe Commerce Cloud を使用している場合、この設定は自動的に処理され、手動で変更する必要はありません。 手動設定は、カスタム CDN、プロキシまたは非標準のヘッダー設定の場合にのみ必要です。
